...
{
	uint8_t fre;
	if(receivedDataBuffer.current_frequency<POWER_MAX_FREQUENCE)
	{
		receivedDataBuffer.current_frequency=receivedDataBuffer.current_frequency+1000;
	}
	else
	{
		receivedDataBuffer.current_frequency = POWER_MIN_FREQUENCE;
	}
	change_out_frequence(receivedDataBuffer.current_frequency);
	fre =  (uint8_t)(receivedDataBuffer.current_frequency/1000);
