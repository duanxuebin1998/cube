...
| V1.30.1.0 | 3 | 存储版本不变；修复UART连续流量中断活锁和双端通信恢复，未改变参数布局、元信息或CRC范围，升级不清现场参数 |
| V1.31.0.0 | 3 | 存储版本不变；协议27采用固定地址并增加LH适配、AO修正和维护继电器能力，原位复用既有参数槽，升级不清其它现场参数 |
| V1.31.1.0 | 3 | 存储版本不变；缩短CPU2看门狗监督节拍并收紧持久参数写门禁，未改变参数布局，升级不清现场参数 |
| V1.32.0.0 | 3 | 存储版本不变；新增固定频率找液位、LH FC01和测量失败传播，未改变`DeviceParameters`结构，升级不清现场参数 |
| V1.33.0.0 | 3 | 存储版本不变；协议28新增命令参数事务和继电器配置校验，参数结构、元信息和CRC范围不变，升级不清现场参数 |
| V1.34.0.0 | 3 | 存储版本不变；协议29调整故障码、远程协议保持和串口参数行为，参数结构不变，升级不清现场参数 |
| V1.35.0.0 | 3 | 存储版本不变；协议30新增编码器故障锁存、FRAM仲裁和24V掉电恢复，参数结构、元信息和CRC范围不变，升级不清现场参数 |
| V1.36.0.0 | 3 | 存储版本不变；协议31增加Newhall扭力模块温度并收紧标定和帧门禁，参数结构不变，升级不清现场参数 |
| V1.36.1.0 | 3 | 存储版本不变；修复CH9141K RSSI半ACK清理与透传恢复，升级不清现场参数 |
| V1.36.2.0 | 3 | 存储版本不变；修复液位标定后的AO量程归一化，升级不清现场参数 |
| V1.36.3.0 | 3 | 存储版本不变；电机目标运动改用动态等待预算，升级不清现场参数 |
| V1.36.4.0 | 3 | 存储版本不变；修复DSM定长收帧和UART6透明链路交接，升级不清现场参数 |
| V1.37.0.0 | 3 | 存储版本不变；协议32增加多参数V4传感器类型和调试结果，复用既有`sensorType`并只追加运行态Input，升级不清现场参数 |
| V1.38.0.0 | 3 | 存储版本不变；增加DSM版本识别和扩展参数读取，新增状态仅驻留RAM，升级不清现场参数 |
| V1.39.0.0 | 3 | 存储版本不变；协议33把AO修正、目标和最近实发电流统一改为`mA x1000`，协议26～32的旧修正值乘10迁移，协议25及更早或未知版本归零；结构大小、地址和CRC范围不变，升级不清其它现场参数 |
| V1.39.1.0 | 3 | 存储版本不变；修正V3/V4/DSM识别路由与身份判据，不改变参数结构、地址、CRC或协议33语义，升级不清现场参数 |
| V1.43.0.0 | 3 | 协议36复用reserved10/11为液位定时矫正开关及周期；结构688字节、CRC偏移684、FRAM A/B槽不变。可兼容的旧协议镜像补0/1440，不恢复出厂；协议33～35的AO x1000修正保留，历史不兼容存储仍沿用原校验规则 |
...
## 2026-08-08 - 新增多参数V4传感器主动与交互协议适配

版本：
- CPU2：`V1.36.4.0 -> V1.37.0.0`（MINOR）。
- CPU3：`V1.36.0.0 -> V1.37.0.0`（MINOR）。

协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`由31提升到32；既有`DeviceParameters.sensorType`新增`MULTIPARAM_V4_SENSOR=15`共享语义，CPU3同步识别、显示和未知值保护。调试Input尾部新增`0x0120~0x0127`四个Float32结果：磁零点电压、动力黏度、运动黏度和传感器供电电压。协议31及更早程序不能解释类型15或新增Input，必须由严格相等门禁阻止与协议32混搭。
- CPU2 `DEVICE_PARAM_VERSION`保持3；`DeviceParameters`结构、大小、字段偏移、`struct_size`、magic、CRC范围和FRAM A/B地址均不变，升级不恢复出厂、不清除其它现场参数。
- 多参数V4协议与多参数传感器通信协议V3.0不兼容；降级前必须先把V4参数65切回交互方式，并把`sensorType=15`改为旧固件可识别的类型，避免旧固件持续收到64字节主动帧。
- CPU3板间轮询与LTD外部FC04共用同一已确认Input快照；外部FC03可读取类型15和协议版本32，外部FC04可读取新增四项。快照未建立、协议不匹配、CPU2断线或补读中返回异常`0x06`，不返回陈旧缓存。
- 协议31的Newhall扭力模块温度槽、共享故障码、既有寄存器、命令和状态语义继续沿用；新增字段位于调试块尾部，不移动既有地址。

本次修改：
- 将原LTD/`DSM_V2_*`通信层统一改名为多参数V3，清理函数、变量、类型和注释中的旧二代命名；V3线上帧格式和寄存器语义不变。
- 新增多参数V4通信层：支持R01=4.0识别、64字节主动帧、CRC16/MODBUS、固定字段校验、错位重同步、序号诊断、8字节交互事务及参数65主动/交互方式切换。
- UART6在中断中只完成收帧和PendSV调度，PendSV完成主动帧解包、校验和运行快照赋值；DMA停收、交互事务和命令切换均补充有界恢复，避免阻塞主循环时主动帧无法更新。
- 新增多参数V4主动测量过程，直接消费最新运行快照；交互方式适配既有密度、液位、水位、姿态、黏度和读取部件参数流程。液位模式发布液位频率，密度模式发布密度谐振频率；测水和磁零点只控制对应使能。
- 参数65切换集中在主动帧后的20～100 ms交互窗口，单次交互总上限3 s；主动帧抢占时保留并优先发布该帧，再等待下一窗口，L/M应答丢失后先读状态字复核事实，不盲目重复翻转。
- 传感器识别先监听V4主动帧，再检查蓝牙、安全协议、V4交互、V3和DSM；命令门禁按实际业务依赖传感器，回零等纯运动流程允许传感器离线执行，附加传感器通道在无有效传感器时跳过。
- CPU3增加类型15的同步和“多参数V4”显示；收到未知类型时保留原始值并告警，批量参数同步拒绝把未知值写回CPU2。CPU2调试数据尾部追加V4磁零点电压、动力黏度、运动黏度和供电电压，CPU3同步扩展轮询块并通过LTD FC04对外提供同一份已确认快照；不改变板间参数结构。
- 同步V4协议卷、LTD共享Modbus协议卷与全量寄存器表、历史版本说明、原始工作簿、需求计划、问题复查、硬件原理图分析和协议索引。

验证：
- `py tools/check_multiparam_v4_protocol.py`通过；覆盖主动帧CRC、固定字段、地址、重同步、序号、两种参数4语义、参数65关闭/恢复及L/M不盲目重试。
- Markdown链接、目标源码编码/CRLF、无替换字符、`git diff --cached --check`、scoped范围门禁和根目录布局比较已通过；提交前按最终版本重新执行。
- 2026-08-08按最终源码执行双端clean-first构建并通过：CPU2于12:15（UTC+8）完成，`text=336876`、`data=2352`、`bss=59376`；CPU3在换行一致性整理后于12:19重新完成，`text=203688`、`data=37972`、`bss=100764`。固定名与`V1.37.0.0`版本名HEX的SHA-256分别一致。

未验证风险：
- 尚未在真实DM4验证500 ms主动周期及抖动、最大命令应答时间、恢复主动后的首帧延时和序号行为、参数65持久化介质寿命、L/M真实互斥行为及物理编号来源。
- 尚未执行真实RS485半双工冲突、DMA错误注入、协议32/31交叉烧写、OLED显示、电机、测水、磁零点和长时间断流恢复；静态检查与构建不能替代目标板、台架、现场或功能安全验证。

## 2026-08-08 - 新增DSM版本识别与扩展参数读取

版本：
- CPU2：`V1.37.0.0 -> V1.38.0.0`（MINOR）。
- CPU3：保持`V1.37.0.0`。

协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`保持32；本次只扩展CPU2到DSM传感器CPU1的维护串口读取，不改变板间寄存器、命令、状态或CPU3对外协议。
- CPU2 `DEVICE_PARAM_VERSION`保持3；`DeviceParameters`结构、大小、字段偏移、CRC和FRAM布局均不变，升级不会恢复出厂或清除现场参数。版本上下文和新增有效标志仅位于RAM。
- V1/V2等不支持`Cv`的设备仅在“`Cv`通信超时且`CV`组合前缀为07”时进入兼容上下文；BCC、帧格式和UART错误保持原错误码。现有`<30.0 pF -> 99999.9 pF`特殊映射和`zero_cap`语义不变，一代仍采用磁零点，本次不新增倾角读取。

本次修改：
- 增加`Cv/CV`版本读取和RAM会话分类，识别V3、V4、V5、V6及历史07前缀兼容设备；版本探测失败不替代既有`CN`传感器识别结果。
- 为`CM`增加27字节正常帧、19字节错误帧的精确收帧和字段校验，读取22.5度周期平方、45度周期平方和动态黏度；`CK`供电电压与`CM`结果在完整解析后设置独立有效标志。
- 读取部件参数时增加DSM的`CK/CM`扩展诊断读取；任一读取失败不阻断核心参数和故障恢复，命令切换仍立即退出，部分成功结果分别输出。
- 收紧低电压日志和`Cv`错误回退，避免把非低压响应误报为低压，也避免V3的BCC、格式或UART错误被07前缀掩盖。

验证：
- `verify-residual-fix.py`、`check_dsm_uart6_frame_contract.py`、`test_dsm_cn_low_voltage_response.py`和`check_dsm_compat_contract.py`通过；目标源码GBK/936、CRLF、无替换字符及`git diff --check`通过。
- 2026-08-08 15:56（UTC+8）按最终V1.38.0.0源码执行`cmake --build build\LTD_MAIN_CPU2 --clean-first`通过；ELF为`text=338988`、`data=2352`、`bss=59440`，固定名与版本名HEX的SHA-256均为`1B8FBB47733871DE82ED05FA066647AC5304126669C692E3F5304F7C3744F4F8`。
- 尚未使用真实DSM覆盖V1/V2无`Cv`、V3坏BCC、现代版本瞬态丢包、CK/CM部分成功、UART错误注入和长时间运行；静态检查与构建不替代台架或现场验证。`check_sensor_fault_contract.py`仍匹配旧`ltd_ret/dsm_ret`代码形状，与当前`Sensor_SelectProbeError()`实现不同步，未作为本提交通过证据。

## 2026-08-10 - 发布协议33、AO千分之一毫安修正与多参数V4交互闭环

版本：
- CPU2：`V1.38.0.0 -> V1.39.0.0`（MINOR）。
- CPU3：`V1.37.0.0 -> V1.38.0.0`（MINOR）。

协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`由32提升到33；AO电流修正、AO运行目标和最近成功下发电流的线值单位由`0.01 mA`改为`0.001 mA`，寄存器地址、字段宽度和有符号属性不变。协议32及更早固件不能按新倍率解释这些字段，双端必须成对升级，LTD/LH上位机也必须按协议33倍率显示和写入。
- CPU2 `DEVICE_PARAM_VERSION`保持3，CPU3 `CPU3_PARAM_VERSION`保持`0x0007`；`DeviceParameters`结构大小、字段偏移、`struct_size`、magic、CRC范围和FRAM A/B地址均不变化。协议26～32保存的旧AO修正值在加载时乘10迁移，协议25及更早或未知协议版本将该槽归零；升级不恢复出厂、不清除其它现场参数。
- AO基础配置中的固定电流、端点电流、仿真电流和故障电流继续使用`mA x100`；最终目标按`base_mA_x100 * 10 + correction_mA_x1000`计算并钳位到`3.200～24.000 mA`。AO禁用态固定`3.400 mA`且不叠加修正。
- 多参数V4主动帧和交互帧格式不变；交互方式增加R02运行状态核对、R04/R05/R06/R07/R11/R12业务读取及R03/R08/R09/R10附加调试读取，既有类型15和Input地址保持协议32布局。

本次修改：
- 将AD5421最终下发、初始化和故障恢复链路统一为`mA x1000`整数换算，使用64位乘法和整数四舍五入映射16位DAC；保留`x100`旧接口并转调新路径，避免浮点换算吞掉`0.001 mA`修正步进。
- 将CPU2 AO修正值、目标、最近实发值、写入判定和恢复目标统一为`x1000`；同步CPU2/CPU3共享寄存器、CPU3参数菜单三位小数、AO运行页三位小数、LTD共享Modbus和LH映射/范围校验。
- 完成旧AO修正值运行期迁移和归一化：协议26～32按旧`x100`值乘10，协议25及更早或未知版本归零，越界值恢复默认0；其它AO参数和现场参数保持原值。
- 完成多参数V4交互测量闭环：读取核心值前用R02核对密度/液位模式及异常位，按业务读取频率、密度、温度、水电容和姿态并执行有限值/范围检查；主动快照也补齐频率、密度和温度边界校验。
- 在V4交互方式下以500 ms节流刷新磁零点电压、动力黏度、运动黏度和供电电压，单项失败保留首个错误且不覆盖已成功结果；传感器公共入口统一转调V4测量层，减少主动/交互分支重复。
- 同步AO、协议33、LTD/LH、CPU3菜单、V4交互语义、参数默认值、历史问题和版本资料；重写仓库README导航并补齐CPU2程序版本与参数存储版本对照。
- 更新CUBE仓库级skill的全部提交、二进制资料、版本记录和可执行测试方案门禁；正式SIL Word仅修复19个内嵌Visio OLE尺寸字段，三份文档65页候选渲染逐页哈希不变；独立F05 Visio源文件同步兼容保存。
- 整理DM4四代数字驱动V6.0原理说明，补充双DDS链路、密度与动力/运动黏度关系；纳入用户已有`CHANGELOG.pdf`和LH Modbus手册更新，不由Markdown自动重生成PDF。

验证：
- AO倍率、AD5421整数换算、CPU3菜单、V4交互协议、外部Input新鲜度和文档契约检查通过；CUBE skill语法/路由、hooks、版本一致性、编码、Markdown链接、根目录布局和Git差异门禁在提交前按最终源码复核。
- 上位机Protocol测试549/549、App测试832/832通过；其17项源码和文档改动位于独立仓库，本次CUBE提交不代替该仓库提交。
- 2026-08-10 13:36（UTC+8）按最终版本执行双端clean-first构建并通过：CPU2 `text=340380`、`data=2352`、`bss=59448`，固定名与V1.39.0.0版本名HEX的SHA-256均为`14B2C3055F51892D6B584F21FA541F4C94C0A1C3D9F091ED71A577DECFA8D1F4`；CPU3 `text=203704`、`data=37972`、`bss=100764`，固定名与V1.38.0.0版本名HEX的SHA-256均为`881C8A6EB9C5C286D16FFC83C7D8D40A9135CDCBC7E428725F956FA30394CDE7`。
- 尚未在真实目标板验证`0.001 mA`设定步进、绝对输出精度、温漂、负载、3.2/24 mA边界、RANGE0/RANGE1硬件配置、断环和恢复；也未执行真实V4传感器、RS485、FRAM掉电、OLED、故障注入或长期现场测试。静态检查、主机测试和构建不替代台架、现场或SIL验证。

## 2026-08-11 - 修正传感器协议识别与故障显示文案

版本：
- CPU2：`V1.39.0.0 -> V1.39.1.0`（PATCH）。
- CPU3：`V1.38.0.0 -> V1.38.1.0`（PATCH）。

协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`保持33；本次不新增或移动共享寄存器、命令、状态及字段，也不改变协议33的AO倍率语义。
- CPU2 `DEVICE_PARAM_VERSION`保持3，CPU3 `CPU3_PARAM_VERSION`保持`0x0007`；参数结构、字段偏移、CRC范围和FRAM布局均不变化，升级不会恢复出厂或清除现场参数。
- 多参数V3、V4和DSM线上协议保持不变；只修正识别路由、识别成功判据和主动V4识别时的临时模式切换。

本次修改：
- 统一通过R01判断多参数传感器代际：V3读取R22识别号，V4读取R67识别号；V3、V4和DSM均要求识别号为正值后才确认类型。
- 主动方式V4在识别阶段临时切换到交互方式读取身份，并在退出前恢复主动方式；普通探测不再发送安全协议HELLO，安全会话留给独立安全测量流程。
- 同步修正CPU2日志、CPU3 OLED、正式Markdown和故障码工作簿中的49项故障名称，并更新协议33与当前固件版本口径。
- 补齐本版本改动与测试方案；任务上下文快照检查脚本的首行元数据与反引号问题已在本机`tools/`同步修复，按仓库边界不纳入本次Git提交。

验证：
- Python语法、CUBE skill、多参数V4、安全协议集成、CPU3故障显示、参数测量故障契约、字库、Markdown链接和Git差异检查按最终工作区执行。
- CPU2、CPU3按最终版本执行clean-first构建；具体结果和版本HEX信息记录在本版本测试方案。
...
## 2026-08-13 - 增加CPU3外部协议只读查询与上位机兼容扫描

版本：
- CPU2：保持`V1.39.2.0`。
- CPU3：`V1.38.2.0 -> V1.39.0.0`（MINOR）。

协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`保持33；本次新增的是CPU3外部COM独立管理帧，不修改CPU2/CPU3共享寄存器、命令、状态或参数语义，CPU2无需修改。
- CPU3 `CPU3_PARAM_VERSION`保持`0x0007`；参数结构、CRC和FRAM布局不变，升级不会恢复出厂或清除现场参数。
- `0x46`既有协议切换目标和`0x00`切换ACK保持兼容；新增操作值`0xFF`只读查询及`0x01`查询响应。旧固件返回`0xC6/0x03`或超时时，上位机仅对当前候选回退旧版业务协议指纹。

本次修改：
- CPU3在统一`0x46 + "LTD"`管理入口增加当前协议查询，返回端口运行态协议原始值；查询返回`HANDLED`，不进入切换接受、FRAM保存或UART重配流程。
- 保持指令切换只修改协议并保留站号和四项串口物理参数；屏幕配置协议时仍应用目标协议的默认串口参数。
- 上位机先用FC02/FC03/FC04确定串口参数和站号，再查询协议；支持两次查询、新旧固件按设备回退、噪声/回显/粘包过滤及保留或未知协议禁止自动连接。
- 同步外部COM协议说明、发现设计边界、协议索引和本机CPU3流程导航。

验证：
- CPU3管理帧C主机测试通过；原协议切换Python回归6/6通过；上位机Protocol测试567/567、连接助手集成测试3/3通过。
...
- `git diff --check`、UTF-8替换字符、C/C++块注释及根目录布局检查通过。全库文档检查仍被既有CPU2流程页断链阻塞，与本次改动无关。
- 尚未执行真实CPU3烧写、RS485、新旧固件混接、FRAM故障注入、UART参数保持波形或现场验证；静态检查、主机测试和构建不替代目标板证据。

## 2026-08-14 - 发布编码器连续性保护、共享故障与通信质量诊断

版本：
- CPU2：`V1.39.2.0 -> V1.40.0.0`（MINOR）。
- CPU3：`V1.39.0.0 -> V1.40.0.0`（MINOR）。

协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`由33升级为34；启用共享故障`12-6 ENCODER_POWERON_CHANGE`，新增`12-17 ENCODER_POSITION_JUMP`。`12-13 ENCODER_LINEARITY_WARNING`继续保留双端定义和CPU3显示映射，当前LIN位只进入诊断统计，不拒绝位置帧、不触发停机。
- 协议33的AO电流修正x1000语义升级到协议34时原样保留；共享寄存器地址、故障寄存器宽度和既有错误码数值不移动。
- CPU2 `DEVICE_PARAM_VERSION`保持3，CPU3 `CPU3_PARAM_VERSION`保持`0x0007`；`DeviceParameters`结构大小、字段偏移、`struct_size`、CRC范围和FRAM布局均不变化，升级不会恢复出厂或清除现场参数。编码轮周长为0的旧存储在运行态归一化为`95.000 mm`，主站写入0时返回非法数据值。

本次修改：
- AS5145保持10 ms定时触发和SPI5 DMA采集，中断只投递固定队列；PendSV按固定预算解析原始帧、分类异常并更新RAM，积压时重新挂起，主循环阻塞不影响连续性判断和异步硬停。
- 上电先用连续3帧、相邻不超过16 counts确认静止单圈角，再与FRAM保存角比较；差值超过320 counts立即锁存`12-6`并撤销累计位置可信度，只有正式回零或零点标定可以重新建立可信基准。
- 运行中按20 m/min、实际采样间隔、编码轮周长、25%余量和8 counts余量计算动态候选增量上限；连续3个超限候选锁存`12-17`。超过50 ms的采样断档只记录一次旧基准异常，同时推进角度和时间基准并撤销位置可信度，避免同一断档被放大为连续3次超时。
- 同一错误码连续3次才锁存普通SSI故障；上电跳变及已经失去累计位置可信度的事件立即锁存。编码轮模式发布故障后直接禁止驱动，电机记步模式保留后台诊断但不让编码器运行故障阻断运动；切回编码轮前检查角度同步、位置可信和故障锁存。
- `ENC?`扩展为编码累计/持久化、SSI原始帧/分类计数和通信质量三行快照，增加候选增量、动态上限、采样间隔、拒绝原因、方向不一致、传输失败、断线、校验和队列溢出等本次上电统计；查询只复制一致RAM快照，不访问SPI或FRAM。
- 加强编码器A/B持久化、紧急掉电回执和FRAM所有权协同：普通保存避让SSI积压并在失败后退避，紧急请求优先提交且有界重试；同步错误名称、不可自动恢复集合、CPU3显示、协议记录、故障码清单和正式工作簿。

验证：
- 基于隔离候选树`280a74808e2fa7375b9d3437f7e4120321fc39de`执行CPU2 clean-first构建通过：`text=341460`、`data=2352`、`bss=59296`；固定名与版本名HEX的SHA-256均为`4687B817108C6BF398E2FAF22DF54A7DA2881CCE1DF58972389A422AF4D5C272`。
- 基于同一候选树执行CPU3 clean-first构建通过：`text=220720`、`data=20996`、`bss=100764`；固定名与版本名HEX的SHA-256均为`121926EB7A6780D20DB4699BB2D2F997C95A127AAFA16242FC4563C2A1D2A6D8`。
- `LTD故障代码统一表.xlsx`完成9个工作表结构和视觉检查，公式错误扫描无匹配；最终工作簿SHA-256为`DE110B17F509F05D6E02DB34032365BE2FFBF55B91D3FAC93142D77FB8E531A3`。

未验证风险：
- 尚未执行真实编码器坏帧、持续LIN、超过50 ms断档、20 m/min双向运动和连续跳变故障注入；动态门限、硬停时序及故障后位置可信度需要目标板和电机台架确认。
- 尚未执行FRAM各阶段随机掉电、真实24 V保持窗口、A/B代次回绕及长期保存压力测试；软件构建不能证明真实掉电窗口内一定完成提交。
- 尚未执行真实OLED、CPU2/CPU3 RS485联调、现场长期运行或SIL验证；静态资料、工作簿检查和clean-first构建不替代目标板、现场和功能安全证据。

## 2026-08-17 - 修正方法5精细调速与AO禁用态硬件门禁

版本：
- CPU2：`V1.40.0.0 -> V1.40.1.0`（PATCH）。
- CPU3：保持`V1.40.0.0`。

协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`保持34；不新增或移动共享寄存器、命令、状态及字段，CPU3无需修改。
- CPU2 `DEVICE_PARAM_VERSION`保持3，`DeviceParameters`结构大小、字段偏移、`struct_size`、CRC范围和FRAM布局不变；升级不恢复出厂、不清除现场参数。
- 方法5继续使用既有目标频率、搜索死区和跟随死区；AO禁用态仍发布禁用来源和名义目标，但不再访问AD5421。

本次修改：
- 方法5首次搜索在第一次跨越目标前保留常规速度；首次换向后以及持续跟随阶段，按超出死区的频差使用二次曲线在`0.0007~0.10 m/min`间连续调速，并在换向前执行慢停。
- 新增不受`0.01 m/min`旧接口分辨率限制的精细速度入口，复用运动前就绪、健康、位置同步、方向和最大速度门禁；低于`0.03 m/min`时跳过不适用的`0.5 mm/s`失步阈值，其余位置、称重、碰撞、通信和命令切换保护继续执行。
- AO上电即禁用或从启用切到禁用时，重置驱动、诊断、恢复、写入尝试、仿真和运行态，不执行AD5421初始化、诊断读取或电流写入；重新启用后仍由原驱动就绪和故障恢复链路接管。
- 同步AO正式说明、版本测试方案和本机程序流程资料；传感器通信源码、协议资料和现场报告不属于本版本提交。

验证：
- 方法5精细速度曲线、连续液位控制、AO输出使能契约和电机运动目标计划四项静态检查通过。
- CPU2 `V1.40.1.0`隔离候选树`a9fe8ee7c197b6b569204102b5efb33933e67171`执行clean-first构建通过：`text=343092`、`data=2352`、`bss=59296`；固定名和版本名HEX的SHA-256均为`F4985EF4056F015F94F91BBB7A13ED44AFC1CD90EC08F509F25118E45292B847`。
- 尚未执行真实电机、液位频率反馈、低速失步、AO SPI静默、AD5421重新启用、目标板、台架、现场或SIL验证；构建和静态检查不替代硬件证据。

## 2026-08-17 - 修正OLED恢复失败闭环与命令确认页额外按键

版本：
- CPU2：保持`V1.40.1.0`。
- CPU3：`V1.40.0.0 -> V1.40.1.0`（PATCH）。

协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`保持34；共享寄存器、命令、状态、故障和字段布局不变。
- CPU3 `CPU3_PARAM_VERSION`保持`0x0007`；参数结构、CRC和FRAM布局不变，升级不会恢复出厂或清除现场参数。
- OLED亮度档位、可见区128×64地址、菜单操作号和命令下发语义保持不变；本次只修正CPU3本地显示恢复和确认按键计数。

本次修改：
- SSD1325硬恢复在复位释放后等待1 ms，初始化后清零完整128×80 GDDRAM，再恢复128×64可见窗口并开屏，避免非可见行残留在后续滚动或异常恢复时重新出现。
- `OLED_Clear`、`OLED_RecoverAndClear`和`all_screen`返回首个HAL失败；显示层在清屏或硬恢复失败时终止当前帧，不继续提交不完整画面。
- OLED恢复每轮最多连续尝试3次；成功后清零计数，失败达到上限后等待60秒恢复窗口、SPI新错误或显式唤醒请求重新开启一轮，避免逐帧硬复位占用总线。
- 命令确认页保留“进入页面的按键只显示页面”保护，页面出现后的下一次确认即执行，修正旧条件导致还需多按一次确认的问题。
- 统一OLED驱动中的SSD1325型号注释，同步版本测试方案和本机程序流程导航。

验证：
- CPU3字库一致性和1109个OLED字符串缺字检查、显示ISR边界、OLED亮度配置、显示命令契约、长按入口及菜单名称宽度检查通过。
- CPU3 `V1.40.1.0`隔离候选树`19033c7fab7683efc4561ee58163a30d4e14ac34`执行clean-first构建通过：`text=220928`、`data=20996`、`bss=100764`；固定名和版本名HEX的SHA-256均为`A655145CD162940084186DE30669695D24B48064C76DCDCB47E87C5163D9FE33`。
- 尚未执行真实OLED完整GDDRAM观察、SPI超时/错误注入、三次失败上限、60秒重试窗口、真实按键确认、目标板、台架、现场或SIL验证；构建和静态检查不替代硬件证据。

## 2026-08-18 - 增加DM4 V4调试、主动帧质量监测与无线滑环兼容

版本：
- CPU2：`V1.40.1.0 -> V1.41.0.0`（MINOR）。
- CPU3：保持`V1.40.1.0`。

协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`保持34；不新增或移动共享寄存器、命令、状态及字段，CPU3无需修改。
- CPU2 `DEVICE_PARAM_VERSION`保持3，`DeviceParameters`结构大小、字段偏移、`struct_size`、CRC范围和FRAM布局不变；升级不恢复出厂、不清除现场参数。
- DM4 V4交互请求暂时统一使用广播地址`0x00`，广播应答允许携带设备自身地址；R67编号`0`按未初始化合法值处理。主动帧固定长度为72字节，并兼容无线滑环在帧内插入连续零字节的现场行为。

本次修改：
- 增加`V4?`、`V4P`、`V4I`、`V4A`、`V4D`、`V4S`、`V4L=0|1`、`V4M=0|1`、`V4O`、`V4R=<n>`、`V4F`、`V4G`和`V4C`串口调试命令，覆盖探测、通信方式、测量模式、互斥功能、原始参数、主动帧和质量诊断。
- V4主动帧解析增加帧内连续零字节兼容、P13/P14扫频周期平方均值解包、有效帧序号丢包统计、最近异常包保存和连续3包异常故障上报；主动切交互在完整主动帧结束后等待至少15 ms，避免半双工冲突。
- V4水位和磁零点按密度模式、互斥和液位模式自动关闭约束统一控制；读取前按需使能，功能阶段结束后保持开启，避免盲目翻转状态。
- 传感器识别和部件参数诊断增加V4/V3/DSM实际TX/RX、长度、HEX和校验阶段打印；修正广播R01应答地址、无线滑环定向抢答和R67编号0识别问题。
- V4浮点NaN映射为`SENSOR_DATA_STALE`；命令15读取姿态角、密度组合量、水位电容或磁零点当前无有效值时保留通信包证据并继续，不把无效值写成0或升级为通信故障。
- 同步V4协议卷、串口调试命令卷、DSM/Safe相关边界说明、无效值梳理和DM4上电识别测试整改记录，并纳入用户维护的对应PDF资料。

验证：
- 目标板日志已证明广播R01、广播R67、编号0身份提交和FRAM A/B保存成功；R07 NaN已确认按非致命无效值继续，R05 NaN已取得原始包证据。
- V4协议、探测日志、串口命令和源码编码检查按最终源码执行；CPU2 `V1.41.0.0` clean-first构建及最终HEX哈希记录在本版本改动与测试方案。
- 尚未执行72字节主动帧长期丢包统计、连续3包异常故障、15 ms半双工切换、水位/磁零点互斥、无线滑环长时间运行、真实Safe模式、现场或SIL验证；静态检查和构建不替代目标板验证。

## 2026-08-19 - 修复无线滑环匹配后传感器重识别与V4探测瞬时13-3上报

版本：
- CPU2：`V1.41.0.0 -> V1.41.1.0`（PATCH）。
- CPU3：保持`V1.40.1.0`。

协议版本/兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`保持34；不新增或移动共享寄存器、命令、状态及字段，CPU3无需修改。
- CPU2 `DEVICE_PARAM_VERSION`保持3，`DeviceParameters`结构大小、字段偏移、`struct_size`、CRC范围和FRAM布局不变；升级不恢复出厂、不清除现场参数。
- 无线滑环RSSI匹配接口保留原有无校验调用；正式命令117增加匹配成功后的传感器识别校验，识别失败时不发布匹配成功状态。

本次修改：
- 正式无线滑环匹配完成后立即调用`SensorService_Detect`，只有蓝牙链路和传感器类型/编号均识别成功才发布最终成功；测量类命令在识别状态无效时重新识别，失败则拒绝继续测量。
- V4主动探测期间启用识别探测模式，保留超时统计和日志，但抑制对非V4传感器预期静默造成的全局连续通信故障`13-3`异步上报；探测结束、切换或超时均清理该模式和质量锁存状态。
- 新增无线RSSI匹配后的校验器接口，保留旧调用的兼容行为；CPU2补丁版本升至`V1.41.1.0`。
...
## 2026-08-26 - 修复V4密度参数无效值不应判为通信故障
...
- CPU2 `DEVICE_PARAM_VERSION`保持3，FRAM布局和现场参数兼容性不变；升级不恢复出厂、不清除现场参数。
- DM4 V4帧格式、交互读写流程和错误码保持兼容；仅调整密度测量参数无效时的上层归一化结果。

本次修改：
- R04无效频率统一输出0，R07无效密度统一输出0，R06无效温度输出`NaN`。
- 传输超时、应答校验、CRC/BCC、地址和模式错误仍原样返回；主动帧过期和参数未更新仍保持`SENSOR_DATA_STALE`。
- 交互读取和主动帧读取采用同一套密度参数无效值处理逻辑。
...
## 2026-09-05 - 新增 CPU3 LED 面板指示灯初始化
...
- CPU3 `CPU3_PARAM_VERSION` 保持 `0x0007`；参数布局、FRAM 数据和启动加载逻辑不变。
- 新增的 PF0/PF10 GPIO 输出只控制 LED 面板指示灯，不改变串口、SPI、OLED 显示和对外协议行为。

本次修改：
- 按当前 LED 面板接线定义 `WLED1=PF10`、`WLED2=PF0`，并在 `MX_GPIO_Init()` 中配置为低速推挽输出、无上下拉。
- 两个引脚在切换为输出模式前写入低电平，当前版本保持常亮；CubeMX `.ioc` 同步保存引脚标签、输出模式和低电平初始状态。
- CPU3 固件版本升级到 `V1.41.0.0`，并补充对应改动与测试方案。

验证：
- `git diff --check`、版本门禁和目标文件 UTF-8/CRLF 检查在提交前执行。
- CPU3 `V1.41.0.0` clean-first 构建通过，62 个目标完成；`text=220960`、`data=20996`、`bss=100764`。固定名和版本名 HEX 的 SHA-256 均为 `067353FD8C41F10F044F0A30A0C45D75B8B6028B4AD82D95B2F19A7111475A17`。
- 已知高电平初始化版本在目标板上未点亮；本次低电平版本尚未烧写验证，仍需确认两灯常亮、引脚映射和上电瞬态。

## 2026-09-05 - 修复方式5换向与变速阶段丢步检测并更新架构资料
...
协议版本/兼容性：
- `DEVICE_PROTOCOL_VERSION` 保持 35；CPU2/CPU3 共享寄存器、命令、状态和 SI 对外协议语义不变。
- CPU2 `DEVICE_PARAM_VERSION` 保持 3，CPU3 `CPU3_PARAM_VERSION` 保持 `0x0007`；参数结构、字段偏移、CRC 范围和 FRAM 布局不变，升级不恢复出厂、不清除现场参数。
- 本次未纳入称重探底可靠性改造；普通液位运动仍保留原有向下绝对称重阈值保护。

本次修改：
- 方式5丢步检测新增基于已下发有效方向和速度的专用窗口，按前一时段指令积分期望位移，换向时重建方向前沿，停止等待不累计期望位移，避免用新方向或旧速度回算上一时段。
- 业务位置、电机模型位置和编码器位置均按指令方向累计“有效前进量”，回退或抖动不重复计入；低于细调丢步检测速度时暂停检测，恢复检测时重建窗口。
- 液位定频闭环将当前有效方向、速度和检测状态传入方式5接口，并在换向停机前结算上一速度时段。
- 纳入当前 SI PLC 对接协议 PDF、CG01 系统架构设计修订和 ER02 软件架构设计文档；SI PDF 二进制变更经逐页栅格比对未发现视觉差异。

验证：
- 隔离候选树执行 CPU2 `V1.42.11.0` Debug clean-first 构建通过，132 个目标完成；`text=362876`、`data=2360`、`bss=59920`。
- 固定名和版本名 HEX 均为 1027398 字节，SHA-256 均为 `CB6EE1AEA44477B8F02A656BCA4E9D3DF3CFD4C2858FCE28A8C0E5DC07A272F0`；BIN 为 365260 字节，SHA-256 为 `DD901E953F4A26B0CB1737AA6D2C2A8607A31F76DE9370E3491E23EDD1F10A12`。
- 本机方式5流程导航已同步并通过严格影响检查；CG01 和 ER02 使用 Word 只读导出逐页检查，未发现正文重叠或裁切。ER02 末页页脚仍显示旧总页数字段，作为已知文档问题保留。
- 尚未执行目标板烧写、真实电机/编码器换向和变速测试、RS485/PLC、故障注入、现场或 SIL 验证。

## 2026-09-07 - 新增液位定时矫正、密度复核与双端配置

版本：
- CPU2：`V1.42.11.0 -> V1.43.0.0`（MINOR）。
- CPU3：`V1.41.0.0 -> V1.42.0.0`（MINOR）。

协议版本/兼容性：
- 共享协议`35 -> 36`，CPU2/CPU3必须成对升级。新增32位保持寄存器`0x0904/0x0906`，分别为刷新开关0/1和周期60～10080分钟，默认0/1440，高字在前；新增共享故障15-34，不复用历史15-32/15-33。
- CPU2存储版本3、CPU3本机参数版本0x0007保持不变，参数结构688字节、CRC偏移684和FRAM布局不变。新字段复用reserved10/11；兼容旧镜像补默认值，保留其它现场参数和协议33～35的AO修正，不因本次升级恢复出厂。历史不兼容存储仍按原规则处理。
- 跟随期间所有持久参数继续禁写；刷新端点、候选、计时和失败计数仅在RAM，不按周期写FRAM。

本次修改：
- 首次稳定停机跟随后启动周期，到期仅锁存请求，稳定、驱动停机且无故障时执行；取消保留请求，改配置或真实故障后重建周期。
- 方法0/1/4/5先采油中再采空气，各10个有效值、每次等待2秒、去两高两低后平均。配置安全中间频率保持独立；方法0/4取中值，方法1/5保留固定目标；严格校验1～6500 Hz、两端差值大于400 Hz及目标两侧200 Hz余量。
- 复用原步进或连续定位算法，候选显式传递；刷新期间保持原有效液位，定位稳定并通过保护后原子提交基准、液位、稳定标志和AO样本。真实探头位置及其它测量通道不冻结。
- 单轮300秒软件预算，采样定位与最多3次尝试共用180秒，旧目标恢复最多120秒；仅旧目标仍可用且重新定位成功才恢复实时跟随，5分钟后重试，连续3轮失败或无法恢复报15-34；具体故障和正常取消分别处理。
- 寻相只在成功下发运动命令后检查丢步，每相独立建窗；合法0频率停机等待后清旧窗口。正常停机采样不做低速误判，已命令运动但实际不转仍检查，位置和碰撞保护保留。
- 方法2不离面、不切模式、不冻结输出，触发当次不计数，后续连续3次稳定读取完成复核，重复缓存也计数；不稳定或无效清计数。方法3保持预留。
- CPU3增加“定时矫正”“矫正周期”菜单、开关枚举及确认、参数同步和15-34原因显示；同步正式协议、故障表新增条目和本机流程资料。本次不纳入称重探底改造、旧故障表合并、项目规范和其它资料整理。
- 对外统一命名“液位定时矫正”，参数全称为“液位定时矫正开关”“液位定时矫正周期”；同步CPU2日志、CPU3显示、协议和售后版本表。仅追加“矫”字28字节点阵及映射，不改旧索引、控制逻辑或参数布局；命名修订并入本次提交，版本不再次递增。

验证：
- 隔离候选树CPU2/CPU3 Debug clean-first构建通过（2026-09-07 17:24:20 / 17:24:24）；CPU2 text=367900、data=2360、bss=60640，CPU3 text=221200、data=20996、bss=100764。
- 刷新编排集成123项、策略136项和独立ARM模块编译通过；两端真实头文件的寄存器、禁写、结构及CRC检查通过，实际迁移函数验证协议33/34/35 AO保留及新参数默认值。
- 连续液位、参数/测量故障、故障目录、CPU3原因显示及菜单短名检查通过；故障表仅追加15-34，原行及其它ZIP部件保留，新增行完成内容和渲染检查。
- 字库494字、1112条显示字符串无缺字，原493字模及索引不变；“定时正周期更新液位”同字体取模与旧字模逐像素一致。“矫”字和新菜单完成点阵预览，尚未验证实机OLED。售后表保留三页及历史记录，仅改本版4个命名单元格。
- CPU2固定/版本名HEX SHA-256：`BECF3BEFE3ED29E92BE158EC46A2DC32442EC1415ECF7E20B4ECDFBF316A40BF`；CPU3：`C7DC90E91731F0F8DBBC55379A0F4D3C7D5AD42BDAFFDDDF0DE5E84204CC3FAA`。
- 未执行Release构建、目标板、真实电机/编码器、传感器与介质、FRAM掉电、RS485、AO/继电器、长期现场或SIL验证；主机模拟不证明真实丢步窗口，300秒不构成硬实时保证。完整步骤及提交边界见对应改动与测试方案。

## 2026-09-07 - 称重探底改用触底减重量并补齐下行低重量保护

版本：
- CPU2：V1.43.0.0 -> V1.44.0.0（MINOR）。
- CPU3：V1.42.0.0 -> V1.43.0.0（MINOR）。

协议版本/兼容性：
- 共享协议36 -> 37，两块板必须配套升级。原0x0216探底扭力阈值名称、地址和已有数值不变，物理含义改为触底减重量；不新增参数，绝对下限固定500原始计数。写入范围0..INT32_MAX，称重探底不允许0；0x0908保持只读预留。
- CPU2 V1.44.0.0对应DEVICE_PARAM_VERSION=3，CPU3本机参数版本0x0007，结构688字节、CRC偏移684及FRAM布局不变。从本次基线升级不清参、不归零、不转换原阈值；更早不兼容存储仍按原规则处理。升级前记录并按减重量复核原设置，回退时恢复旧含义的设置并配套回退双板。

本次修改：
- 预计罐底前300mm进入检测窗口并冻结已有低通称重基准，以当前称重不大于基准减阈值作为候选；入口要求当前值、候选下限均高于500且当前值高于候选下限，首次建基准不触发。
- 粗找和精找均在距参考位置300mm内限速0.50m/min、30mm内限速0.10m/min；记录制动前触发位置，上提补偿过行程并留200mm余量，允许单侧5mm短行程，两次精找相差不超过10mm后取均值。
- 每次精找前上提结束须在1000ms内收到新有效称重帧且回到固定候选下限及500以上；无新帧不得使用旧缓存通过。保留通信、取消和运动故障出口，不增加额外微动或重建低基准。
- 通用下行碰撞以当前重量不大于500或相对变轻超过比例阈值触发；忽略区、停止态、通信优先及上行释放规则不变，专用探底候选优先于通用碰撞。普通定频运动不再复用探底参数作绝对阈值。
- 仅业务未找到罐底允许原重试；称重入口、释放、通信和运动错误不能被参考罐高兜底掩盖，兜底不发布真实罐底有效。编码器修正同步底部参考坐标。
- SI按称重200mm/陀螺100mm释放位置建立Point0；40001首点不高于Point0或超出罐高时拒绝计划，不再静默更改首点。
- 同步双端参数说明、协议37资料、当前探底方案、售后表及本机流程资料。保留9月3日PDF为历史方案，不作为当前实现依据；其他任务的规范与资料改动不纳入。

验证：
- 2026-09-07双端Debug clean-first通过，无编译警告/错误；CPU2 text=370140、data=2360、bss=60672，CPU3 text=221200、data=20996、bss=100764。
- 生产函数主机回归覆盖新旧帧、通信/释放/取消/计数回绕、固定基准、下行500边界、相对碰撞、忽略区、候选优先及SI；双端实际头文件协议/地址/权限/持久化分类检查、SI共享契约与OLED字库检查通过。
- 售后表完成新增行和提醒渲染、历史单元格与格式比对；本次局部修改不带入其他任务的页签整理。
- 固定频率入口仍有旧阈值与零点绝对重量比较，保留为已知限制；“基准减阈值必须大于500”保持当前实现，尚未另行放宽。目标板噪声、真实制动、RS485、FRAM掉电、Release构建、现场及SIL均未验证。详见同版本改动与测试方案。
