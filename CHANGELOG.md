...
本次修改：
- 修复丢步后零点标定脱离零点期间的称重误报：回零/标零内部退让和脱离动作改用专用 `Zero_MoveDownWithoutWeightGuard()`，调用 `MotorCtrl_MoveBlockingNoDetectQuiet()` 跳过通用称重碰撞误判，并在动作完成后用 `Weight_RebaseStableWeight()` 对齐稳定称重基准。
- 为零点退让/脱离路径保留业务日志，只打印阶段名、下行距离、当前称重和位置参考，不恢复底层无检测运行的密度/温度/频率刷屏。
