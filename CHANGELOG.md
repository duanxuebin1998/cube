...
协议版本与兼容性：
- CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`由31提升到32；既有`DeviceParameters.sensorType`新增`MULTIPARAM_V4_SENSOR=15`共享语义，CPU3同步识别、显示和未知值保护。协议31及更早程序不能解释类型15，必须由严格相等门禁阻止与协议32混搭。
...
- 协议31的Newhall扭力模块温度槽、共享故障码、其它寄存器、命令和状态语义继续沿用。
...
- CPU3增加类型15的同步和“多参数V4”显示；收到未知类型时保留原始值并告警，批量参数同步拒绝把未知值写回CPU2。CPU2调试数据尾部追加V4最新测量结果，不改变板间参数结构。
- 同步V4协议卷、历史版本说明、原始工作簿、需求计划、问题复查、硬件原理图分析和协议索引。
