...
版本：
- CPU2: V1.7.3.0 -> V1.9.0.0
- CPU3: V1.5.0.0 -> V1.6.0.0

协议版本/兼容性：
- `V1.8.0.0` 是 CPU2 无线滑环本地调试能力的阶段性开发口径，本次不单独发布；最终与 CPU3 正式命令和 MAC 显示合并为一个版本提交。
- 新增 CPU2/CPU3 共享命令 `CMD_PAIR_NEAREST_WIRELESS_SLIPRING = 117`，并在输入寄存器末尾追加无线滑环匹配结果状态，协议版本从 3 升级到 4。
- CPU2 V1.9.0.0 与 CPU3 V1.6.0.0 配套使用；旧 CPU2 不识别命令 117，协议版本不匹配时应按状态页提示处理。
