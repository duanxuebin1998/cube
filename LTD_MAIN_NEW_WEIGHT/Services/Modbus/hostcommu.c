    ...
    // 处理有效请求
        ...
        // 处理支持的合法请求
            ...
            switch (functioncode) {
                ...
                case FUNCTIONCODE_WRITE_COIL:         // 05 - 写单个线圈
                    HCOM_SendCount = Response05Process(rcvbuff, HCOM_SendBuff);
                    break;
         ...
         HAL_UART_Transmit(&huart2, HCOM_SendBuff, HCOM_SendCount, HAL_MAX_DELAY);  // 通过UART发送响应帧
    }
}
