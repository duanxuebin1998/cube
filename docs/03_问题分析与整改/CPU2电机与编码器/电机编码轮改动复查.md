...
- `docs/07_硬件原理图/00_原始资料/CPU2芯片手册/TMC5130A_datasheet_rev1.21.pdf`

整理内容：

1. CPU2/CPU3 参数打印改为中文分组格式。
2. 故障打印字段改为中文，命令切换时进入统一错误处理，停止策略更多使用 `motorSlowStop()`。
3. 液位、水位、罐底、罐高、密度、传感器、扭力、Modbus 通讯等日志改为中文描述。
4. 保留必要技术词：`CRC`、`BCC`、`FRAM`、`TFIT`、`XACTUAL`、`XTARGET`、`VMAX`、`COM1/COM2/COM3`、`UART6` 等不翻译。
5. 故障代码表按当前 `ErrorCode` 枚举重新整理，补齐新增电机、编码器、密度测量等故障码；当前维护入口已合并为 `docs/00_构建与版本/故障码/LTD故障代码统一表.xlsx`。
...
- `docs/00_构建与版本/故障码/LTD故障代码统一表.xlsx`
- `docs/07_硬件原理图/00_原始资料/CPU2芯片手册/TMC5130A_datasheet_rev1.21.pdf`
- `docs/03_问题分析与整改/CPU2电机与编码器/电机编码轮改动复查.md`
