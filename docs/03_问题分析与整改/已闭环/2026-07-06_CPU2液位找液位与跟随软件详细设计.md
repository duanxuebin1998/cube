...
| `5` | 固定频率速度闭环 | 不粗找端点，校验 `oilLevelFrequency` 为 `1..6500 Hz` 以及位置/称重边界后进入 `FrequencyLevel_RunFixedClosedLoop(SEARCH)`。 | `FrequencyLevel_RunFixedClosedLoop(FOLLOW)`。 | 直接使用已知目标频率，不学习 `oil_frequency`，不做命中后的固定下行 100 mm。 |
...
| `g_deviceParams.oilLevelThreshold` | 方法 `2/4/5` 的首次查找死区；方法 `5` 按 x10 换算为 Hz。 |
| `g_deviceParams.oilLevelHysteresisThreshold` | 方法 `2/4/5` 的持续跟随死区；方法 `5` 按 x10 换算为 Hz。 |
...
4. 如果是方法 `5`，清运行态后直接使用 `oilLevelFrequency` 进入固定频率速度闭环查找并返回；不执行后续端点粗找，也不读取 `oil_frequency`。
...
上述命中后继续下行 100 mm、读取 `oil_frequency` 的流程只服务方法 `0/1/4` 的端点粗找。方法 `5` 在 `OilLevel_TryRunDirectSearchMethod()` 已提前分流，不进入 `SearchOil()`，因此不会为了“全部振动管入液”执行额外下行。
...
| 保护 | 命令切换、读取频率失败、位置越界、盲区等待、扭力碰撞、丢步检测；方法 `5` 另外在运动前后检查明确位置和称重上下限 |
...
- `FrequencyLevel_ValidateFixedConfig()` 校验 `oilLevelFrequency`、盲区/罐高、满载称重、罐底称重和零点阈值关系。
- `oilLevelFrequency == 0` 返回参数缺失，`oilLevelFrequency > 6500 Hz` 返回参数越界。
- 校验成功后以局部目标进入 `FrequencyLevel_RunClosedLoopConfigured()`；公开运行态 `follow_frequency` 同步为本轮目标，但不修改 `DeviceParameters` 或 FRAM。
- 首次搜索死区来自 `oilLevelThreshold`，持续跟随死区来自 `oilLevelHysteresisThreshold`，未配置时均使用 15 Hz。
- `current_frequency > target + deadband` 时下行，`current_frequency < target - deadband` 时上行；速度从 `0.10 m/min` 起按超出死区的频差比例增加，并受默认速度上限约束。
- 连续 3 次进入死区后停机记录液位；首次搜索完成后直接返回，不追加固定下行 100 mm。
- 首次搜索到达 `blindZone` 时停机并返回 `MEASUREMENT_OILLEVEL_LOW`；持续跟随到达盲区时停机等待，仅当当前频率低于目标死区下沿时允许上行撤回。
...
| 固定频率或安全边界参数非法 | 方法 `5` 在启动运动前返回参数缺失或参数越界错误。 |
| 密度无效 | 连续无效达到限制后返回 `DENSITY_INVALID`。 |
| 位置高于罐高上限 | 清命中/稳定标志，慢停，返回 `MEASUREMENT_OILLEVEL_HIGH`。 |
| 方法 `5` 首次搜索到达盲区 | 清命中/稳定标志，停机并返回 `MEASUREMENT_OILLEVEL_LOW`，不继续下行。 |
| 方法 `5` 持续跟随到达盲区 | 清命中/稳定标志并停机等待；只在频率低于目标死区下沿时上行撤回。 |
| 扭力碰撞 | 停止闭环并返回底层错误码。 |
| 丢步检测失败 | 停止闭环并返回底层错误码。 |
...
说明：并入 CPU2 `V1.21.6.0` 主固件提交的未用代码清理已删除未调用的 `determine_level_status()` 旧导出包装，当前对外保留 `determine_level_status_motion()` 供运动过程状态判定调用；详细改动与检查项统一维护在 `2026-07-10_CPU2_V1.21.6.0_CPU3_V1.20.0.0_传感器编号与CPU2通信失败闭环_改动与测试方案.md`。
...
| 方法 `5` 固定频率速度闭环 | `oilLevelFrequency` 在 `1..6500 Hz`，位置和称重边界有效 | 执行找液位/跟随 | 不粗找端点、不学习油中频率、不追加下行 100 mm；按频差方向和大小闭环，连续 3 次进入对应死区后记录液位。 |
| 固定频率非法 | `oilLevelFrequency` 为 0 或大于 6500 | 执行方法 `5` | 返回参数缺失或参数越界，不启动闭环。 |
| 方法 `5` 首次搜索到盲区 | 传感器位置到达 `blindZone` | 执行找液位 | 停机并返回 `MEASUREMENT_OILLEVEL_LOW`，不继续下行。 |
| 方法 `5` 跟随到盲区 | 传感器位置到达 `blindZone` | 执行跟随 | 停机等待；当前频率低于目标死区下沿时上行撤回，其它情况不运动。 |
...
- 方法 `5` 不应重新加入空气/油中粗找或命中后的固定下行；它的目标来自 `oilLevelFrequency`，不依赖 `oil_frequency`。
