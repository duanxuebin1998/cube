...
> 当前复核（2026-07-08）：状态：已闭环。当前 HEAD 已通过 `65ad08a8` 落地启动早期 `MotorCtrl_BootSafeStop()`、编码器先初始化/首帧门禁和启动错误保留；现行 `main.c` 在 `App_Init()` 前调用停机，`app_main.c` 会在编码器未就绪或启动错误时阻止默认命令。本文后续验证清单仍作为现场复测依据保留。
...
| `LTD_MAIN_CPU2/BSP/Peripherals/src/AS5145.c:269-317` | TIM1 到期后才发起 SSI DMA 读取，DMA 回调成功后才 `Update_Encoder_Count()`。 | `Start_Encoder_Collection_TIM()` 返回成功只表示定时器启动，不表示编码器已经有有效首帧。 |
| `LTD_MAIN_CPU2/Services/Encoder/encoder.c:126-143` | 单次角度差按 `MAX_ANGLE / 2` 处理跨零点。 | 启动盲区内如果运动超过半圈，无法从旧角度和新角度推回真实圈数。 |
| `LTD_MAIN_CPU2/BSP/Peripherals/src/TMC5130.c:499-513` | `stpr_stop()` 会写 `VMAX=0`、读 `XACTUAL`、写 `XTARGET=XACTUAL`、切回位置模式。 | 这是已有的安全停机能力，但启动流程没有足够早调用。 |
...
复位后的第一优先级应该是让电机驱动处于确定的安全态，但当前 `MX_GPIO_Init()` 直接把 `DRV_ENN` 写成使能态，`App_Init()` 又没有先执行低层停机。真正清 TMC 旧目标的动作要等到 `MotorCtrl_Init()`，而它排在 1 秒延时、参数、编码器、HART、扭力、Modbus、AD5421 之后。
