...
当前共享协议版本为 `DEVICE_PROTOCOL_VERSION = 14`。本次只让 CPU3 的外部 COM 口实际响应既有共享协议，没有增加、移动或改变 CPU2/CPU3 共享字段，因此协议版本保持 `14`。
...
- `DEVICE_PROTOCOL_VERSION` 保持 `14`，因为共享地址、字段、命令和值语义均未变化。
