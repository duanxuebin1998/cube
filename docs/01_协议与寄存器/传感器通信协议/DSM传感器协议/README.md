...
当前实现差异、风险等级和关闭条件见 [DSM 传感器协议适配复查](../../../03_问题分析与整改/未处理/2026-07-18_DSM传感器协议适配复查.md)。该报告已于 2026-07-20 按 V5.4 工作区重新复核；它仍是静态证据，不代表问题已经整改或完成真实 DSM 台架验证。
...
| `CD` | 切换 DSM 密度模式 | 已实现 |
| `Cd` | 读取频率、密度、温度 | 已实现 |
| `Cl` | 读取水测量电容 | 已实现；检查 11 字节、BCC 和 CRLF，但当前会把合法的 `<30.0 pF` 改写为 `99999.9 pF`，属于 `DSM-CUBE-01` P1 问题 |
| `CB` | 切换 DSM 液位模式 | 已实现 |
| `Cb` | 读取液位频率 | 已实现 |
| `CN` | 读取传感器/振动管编号 | 已实现 |
| `Ch` | 读取陀螺仪 X/Y 角度 | 已实现 |
...
- V5.4 的 CUBE host profile 与当前 `Ch` 参数静态一致：500 ms 单次等待、最多 3 次、重试前 300 ms、每次尝试前清理旧帧。冷唤醒首次超时属于协议允许路径，但 CPU2 会打印“错误重试/重试成功”，形成 `DSM-CUBE-09` 诊断噪声；第二次成功时限仍待台架证明。
