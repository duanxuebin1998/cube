...
本目录保存 CPU2 与传感器之间的内部通信协议资料，包括传统 DSM 通信、多参数传感器通信协议、新一代安全通信协议卷、SIL 设计理由和验证证据映射。

## 统一命名

- 协议正式名称统一为“多参数传感器通信协议”。
- `多参数传感器通信协议 V3.0`：当前用于 LTD 传感器。
- `多参数传感器通信协议 V4.0`：当前用于 DM4 传感器。
- LTD、DM4是传感器类型，不是协议名称。V3.0与V4.0属于同一协议的不同版本，但参数地址和部分模式命令不兼容。
- R01只表示协议版本。当前R01为`3.0`时对应现有LTD，R01为`4.0`时对应现有DM4；以后LTD采用V4.0时，不能再用R01区分LTD和DM4。

## 文档索引

| 文档 | 内容 |
| --- | --- |
| [多参数传感器通信协议历史版本说明.md](DM4传感器协议/多参数传感器通信协议历史版本说明.md) | 汇总2020年至2026年V1.0～V4.0修订，说明当前LTD使用V3.0、当前DM4使用V4.0，以及版本识别和类型识别的边界 |
| [多参数传感器通信协议V4.0.md](DM4传感器协议/多参数传感器通信协议V4.0.md) | V4.0正式正文；当前实现对象为DM4，参数地址和`D/L/M/S`命令语义不兼容V3.0，并定义64字节主动上报帧 |
| [DM4传感器协议/README.md](DM4传感器协议/README.md) | DM4当前采用V4.0的资料入口和原始工作簿索引；DM4是传感器类型，不作为协议正式名称 |
...
- 多参数传感器通信协议按版本维护；当前LTD使用V3.0，当前DM4使用V4.0。不得跨版本套用参数地址或模式命令，也不得用协议版本号长期代替传感器类型标识。
