...
协议文档中密度缩放写为 `0.01 kg/m3`，示例 `450.00 -> 45000`。协议 13 前适配层曾将内部 `kg/m3 x10` 转为协议 `kg/m3 x100` 输出；协议 13 后 CPU2/CPU3 内部密度 raw 已统一为 `kg/m3 x100`，SI7000 层不再额外乘 10。由于 16 位寄存器最高只能表示 `655.35 kg/m3`，超过范围时钳位到 `65535`，该限制仍需 PLC 侧确认。
...
| `30003` | Current Density | 协议 13 后内部与 SI7000 协议均按 `kg/m3 x100`，超过 16 位时钳位 |
