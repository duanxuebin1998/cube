...
- 旧程序语义：保留字段，默认值为 `0`
- 当前工作区目标：协议版本 `36`
- 当前实现基线：协议版本 `36`，CPU2 `V1.43.0.0` / CPU3 `V1.42.0.0`
- 本次提交基线：协议版本 `36`；现场部署和真实总线验证尚未执行
- 当前语义：在协议35基础上增加液位定时矫正开关、周期和共享故障15-34；CPU2/CPU3仍按协议版本严格相等判断。DM4类型14及Safe预留模式语义不变。
