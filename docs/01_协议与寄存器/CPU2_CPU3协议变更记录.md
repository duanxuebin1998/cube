...
- 旧程序语义：保留字段，默认值为 `0`
- 当前协议版本：`37`，配套 CPU2 `V1.44.0.0` / CPU3 `V1.43.0.0`。
- 本次提交基线：协议版本 `36`，CPU2 `V1.43.0.0` / CPU3 `V1.42.0.0`；现场部署和真实总线验证尚未执行。
- 当前语义：协议37将原探底扭力阈值改为触底减重量，绝对重量下限固定500，不新增参数；保留协议36液位定时矫正能力。CPU2/CPU3仍按协议版本严格相等判断。
...
| 31 | V1.36.0.0 | V1.36.0.0 | 200 | 复用输入寄存器`0x0116~0x0117`的未使用扭力参数槽，改为Newhall扭力模块温度IEEE754 float32原始位，高16位在前；地址、宽度、轮询块和参数存储布局不变。CPU2 V1.36.1.0～V1.36.2.0继续沿用协议31。 |
| 32 | V1.37.0.0 | V1.37.0.0 | 200 | `sensorType`新增15“多参数V4”；调试Input尾部追加`0x0120~0x0127`四个Float32结果：磁零点电压、动力黏度、运动黏度和传感器供电电压。CPU3板间轮询和LTD外部FC04共用同一已确认快照；协议31及更早程序与外部主机资料不包含这些语义。 |
| 33 | V1.39.0.0 | V1.38.0.0 | 200 | AO第4个32位槽、LTD `0x0606~0x0607`和LH `0x0018~0x0019`地址均不变；`current_correction_mA_x1000`按Int32补码、0.001mA、-1000~+1000解释，AO最终目标和最近实发值同步改为x1000。基础AO配置仍为x100。协议26～32旧修正值加载时乘10迁移，协议25及更早或未知版本归零。`DEVICE_PARAM_VERSION=3`、结构尺寸和CRC范围不变。 |
| 34 | V1.40.0.0 | V1.40.0.0 | 200 | 启用12-6编码器上电位置跳变并新增12-17编码器运行位置跳变；12-13继续保留定义和显示映射，LIN仅作诊断统计。故障寄存器地址和宽度不变；协议33的AO x1000修正值升级时原样保留；参数结构、`DEVICE_PARAM_VERSION=3`和CPU3本机参数版本`0x0007`不变。 |
| 35 | V1.41.0.0 | V1.40.1.0 | 200 | 将同一物理DM4传感器统一为`sensorType=14`，V4为普通运行协议，Safe为预留运行协议模式；协议32～34持久化的旧值15由CPU2启动时迁移并写回14。字段地址、宽度、参数结构、`DEVICE_PARAM_VERSION=3`和CPU3本机参数版本`0x0007`不变。 |
| 36 | V1.43.0.0 | V1.42.0.0 | 200 | 复用reserved10/11为液位定时矫正开关与周期，LTD保持寄存器0x0904/0x0906；新增15-34液位定时矫正失败。默认关闭，液位跟随禁写，结构和存储版本不变。 |
| 37 | V1.44.0.0 | V1.43.0.0 | 200 | 原探底扭力阈值0x0216改为相对起始重量的减重量，名称、地址和值保留；绝对下限固定500，不新增参数。结构、CRC范围及存储版本不变。 |
...
- 原 `bottom_weight_threshold`、名称“探底扭力阈值”和共享/LTD保持寄存器 `0x0216~0x0217` 均保留，只将物理含义改为触底减重量，高16位在前，单位为原始称重计数。允许写入 `0..2147483647`；称重探底时必须为正，角度模式不受影响。
- 绝对重量下限在CPU2内部固定为500，不增加参数或菜单。开始检测时，当前称重必须高于500；基准减去减重量必须仍高于500，且入口当前称重必须高于新候选下限。
- 基准采用进入位置窗口时已有低通 `stable_weight` 并在本次粗找、两次精找中固定。后续 `current_weight <= 基准 - 减重量` 才构成候选；首次建基准这一拍不是候选。释放后须恢复到固定候选下限以上才允许精找，不随卸载降低基准。
- CPU3菜单、参数镜像、写入ACK与CPU2存储链路沿用现有机制。值超过INT32_MAX返回异常03，运行忙时沿用异常06，地址0x0908及后续保留槽仍不可写。
- 升级保留原阈值数值并按减重量使用，出厂值仍为500；不清参、不转换或归零该值，现场需按新含义复核。`DeviceParameters`尺寸、CPU2参数版本3、CPU3本机参数版本0x0007均不变。上一轮复用reserved18的方案仅存在于未发布工作树，已取消，无须为其中间格式增加迁移逻辑。
- 称重入口、释放、通信和运动保护错误不允许被内部重试或参考值回退转成成功；“未找到候选”业务错误仍沿用既有重试和兼容回退。
- 两端必须同为协议37，协议36/37混用由严格版本门禁拦截。本记录随 CPU2 V1.44.0.0 / CPU3 V1.43.0.0 提交纳入，提交哈希以 Git 记录为准；尚无目标板、RS485及现场证据。

### 协议版本 36

关联改动：

- 原`reserved10`定义为`liquidLevelReferenceRefreshEnable`，LTD/板间保持寄存器`0x0904~0x0905`，UInt32枚举0=关闭、1=启用，默认0。
- 原`reserved11`定义为`liquidLevelReferenceRefreshIntervalMin`，寄存器`0x0906~0x0907`，UInt32分钟，范围60~10080，默认1440。均高16位在前。
- 两项均为CPU2持久参数。沿用液位跟随期间所有持久参数禁写的原门禁，不提供延迟写入；合法地址但值越界返回异常03，跟随忙时返回06。
- 方法0/4重新采集空气和油中频率并使用新中值；方法1/5采集两端点但保持配置的固定目标；方法2沿用原密度跟随，触发当次不计数，后续连续3次稳定读取完成复核，重复缓存数据也计数，不额外判断数据新旧；方法3不参与。
- 频率法刷新期间保持上次有效液位及液位派生输出，不将中间探头位置发布为液位。成功定位后原子提交；有效旧目标回退并重新找到当前液面后恢复实时跟随，5分钟后重试。
- 台架初始预算：单轮300秒，采样和定位共用前180秒，回退最多120秒且包含在总预算内，单轮最多3次尝试（含首次）；连续3轮失败或无法有效回退报故障。传感器、电机、位置等具体故障直接透传，命令取消不计失败。
- 新增`MEASUREMENT_LEVEL_REFERENCE_REFRESH_FAILED=0x000F0022`，显示15-34“液位定时矫正失败”。不复用冻结的历史15-32/15-33，不增加故障寄存器。

兼容与验证边界：

- CPU2、CPU3必须同时使用协议36。共享地址既有部分不移动，`DeviceParameters`尺寸、CRC范围、CPU2`DEVICE_PARAM_VERSION=3`和CPU3本机参数版本`0x0007`不变。
- 旧协议加载时，仅将新启用的两个保留槽初始化为0/1440；保留其它现场参数。AO修正值按协议33~35既有x1000语义原样保留，不因升级36清零。
- 端点、刷新计时、失败计数和候选数据仅在RAM中；周期刷新不保存FRAM。调试器单次请求不是生产Modbus命令。
- 单轮预算按主线程检查点执行；同步通信和停机确认的最坏耗时尚需台架测量，300秒不构成已验证的硬实时或SIL保证。本次固件组合为CPU2 V1.43.0.0 / CPU3 V1.42.0.0，未执行现场部署。

### 协议版本 34

关联改动：

- AS5145继续解析并逐帧累计 `LIN` 位；当前正式策略关闭LIN故障判定，LIN位本身不拒绝位置帧、不锁存 `12-13 ENCODER_LINEARITY_WARNING`。
- 上电先确认连续3帧稳定单圈角，再与FRAM保存角比较；超过接纳阈值时立即锁存 `12-6 ENCODER_POWERON_CHANGE`，累计位置置为无效，仅允许回零或零点标定重新建立基准。
- 运行期按20 m/min极限速度、编码轮周长和实际采样间隔动态计算单帧物理上限，统一包含25%余量和8 counts余量，最长连续展开间隔50 ms；16 counts只用于上电三帧稳定确认，不是运行门限。
- 连续3个超物理上限候选帧锁存 `12-17 ENCODER_POSITION_JUMP`；被拒绝样本不更新 `g_encoder_count` 和可信 `prev_angle`。方向不一致只累计到 `ENC?` 诊断统计，本身不拒绝位置帧、不触发停机。
- `ENC?` 增加候选增量、动态上限、采样间隔、拒绝原因、拒绝总数、稳定帧数和同步状态，便于现场区分传感器状态位、采样间断、物理跳变和方向异常。
- `12-6`、`12-17` 禁止通用故障自动重跑；`12-13` 保留CPU2错误名称和CPU3显示映射，但当前程序不主动产生。

兼容影响：

- 共享故障码集合和解释已变化，CPU2与CPU3必须同时使用协议34，协议33及更早CPU3不能解释12-17。
- 故障寄存器地址、字段宽度和参数布局不变；协议33升级到协议34时保留现有AO `current_correction_mA_x1000`，不清零现场修正值。
- 本轮未修改CPU2 `DEVICE_PARAM_VERSION=3`或CPU3本机参数版本`0x0007`，不因本次协议升级清除现场参数。

### 协议版本 35

关联改动：

- `DeviceParameters.sensorType`当前合法值收敛为12=DSM、13=多参数V3、14=DM4；协议32～34使用的15不再作为当前合法类型。
- DM4普通上电识别只走V4主动帧、公共R01和R67链路，不执行Safe `HELLO`探测。Safe协议保留为DM4的运行模式，进入、退出和业务切换接口尚未冻结，当前不自动激活。
- CPU2新增传感器服务接口和驱动注册表：上层测量只依赖`SensorService_*`，驱动注册表按持久化类型与运行模式选择DSM、V3、DM4 V4或预留DM4 Safe驱动。
- CPU2加载协议32～34或其它合法旧镜像时，如发现`sensorType=15`，在运行期迁移为14并通过既有FRAM A/B保存路径写回；迁移不改变传感器编号。
- CPU3同步只接受14作为DM4当前类型，并显示“DM4传感器”；15仅保留为协议32～34历史文档值，不提供运行期别名。

兼容影响：

- CPU2与CPU3必须同时使用协议35。协议34及更早CPU3会把14解释为旧Safe类型或把15解释为V4，禁止与协议35 CPU2混用。
- `sensorType`地址、32位宽度和结构偏移不变；`DeviceParameters`结构大小、`DEVICE_PARAM_VERSION=3`、CRC范围和FRAM A/B布局不变，不触发整套参数恢复出厂。
- 从协议32～34升级时，只对旧`sensorType=15`做定向迁移并保存；其它现场参数保持原值。降级到协议34及更早版本前必须重新确认传感器类型语义，不能直接复用协议35保存的14。
- Safe模式尚未形成对外可用的切换流程，协议35不代表Safe通信已经启用或通过功能安全认证。
...
主题：复用未使用的扭力参数调试槽，向CPU3发布Newhall扭力模块温度，并在扭力标定等待页和读取部件参数结果页显示。

#### 字段语义

- 输入寄存器`0x0116~0x0117`地址和32位宽度保持不变，原“扭力参数”占位语义替换为“扭力模块温度IEEE754 float32原始位”，Modbus高16位在前。
- CPU2/CPU3测量结构字段统一命名为`torque_temperature_bits`，共享寄存器宏统一命名为`REG_DEBUG_TORQUE_TEMPERATURE_BITS`；不保留旧名称别名，避免后续代码继续误用“参数”语义。
- CPU2对UART4候选帧统一校验精确22字节、帧头`CC 03 83`和前21字节异或BCC；校验通过后才更新重量、刷新扭力通信接收时间并解析温度。截断帧、粘连帧、错误帧头和BCC错误帧均不得更新重量或温度，也不得清除扭力通信超时。
- 合法帧中的温度指数为全1时不更新温度槽；中断中只组合原始位，不执行浮点运算、打印或阻塞。
- 上电或重新初始化扭力模块后，该槽先发布安静NaN位模式；CPU3拒绝NaN、无穷和超出`-999.99~999.99 ℃`显示范围的值，并显示`--.--`。
- CPU3在“获取空载/满载扭力中”页面第二行显示温度，保留第一行状态、第三行实时扭力和第四行返回操作；`STATE_READPARAMETEROVER`读取部件参数结果分页新增“扭温”项目，无效时显示`--.--`。

#### 兼容性和存储边界

- 协议31改变既有共享槽的字段含义，与协议30及更早程序不兼容；CPU2和CPU3必须严格使用相同`DEVICE_PROTOCOL_VERSION=31`。
- 不新增、不删除、不移动共享寄存器，不扩大调试轮询块，不改变`DeviceParameters`、`DEVICE_PARAM_VERSION=3`、CPU3本机参数版本或FRAM布局。
- 协议31首个正式固件组合为CPU2 `V1.36.0.0` / CPU3 `V1.36.0.0`，两端必须严格配套；协议30及更早组合由严格版本门禁拦截。

#### 验证边界

- 静态检查和双端构建需覆盖地址不变、双端协议版本一致、Newhall温度字节序、21字节截断帧、超过22字节粘连帧、错误帧头、BCC错误、NaN/Inf拒绝，以及扭力标定等待页和读取部件参数结果页的CPU3两位小数与无效占位。
- Newhall温度通常落后称重一轮，首帧可能为0 ℃；当前帧格式依据协议资料和模块源码，仍需真实UART4抓包确认float32字节序，并在实机OLED验证刷新、负温和边界显示。

#### 已确认延期问题（2026-07-29）

- **扭力温度缺少超时失效机制，暂不处理。** `torque_temperature_bits`当前只在扭力初始化时置为安静NaN、在合法Newhall温度帧到达时更新；通信超时、坏帧或断链路径不会把已有温度重新置为无效。CPU3仅按IEEE754有效性和显示范围判断，未校验更新时间，因此旧温度可能继续保留在共享快照中。后续处理时应增加温度更新时间或有效标志，并在超时后发布NaN。
- **读取部件参数不保证取得本轮新温度，暂不处理。** `Sensor_ReadPartParamsInternal()`当前只调用`Weight_CheckOwnCommunicationTimeout()`判断最近一次扭力接收是否仍在1 s窗口内，不记录命令开始时刻，也不等待命令开始后的新Newhall温度帧；`STATE_READPARAMETEROVER`结果页可能先消费此前缓存的温度。后续处理时应以温度帧代际或时间戳证明结果来自本轮读取，未取得新温度时显示`--.--`，且不影响其它部件参数完成。

### 协议版本 32

主题：增加多参数V4类型15，并把四项V4测量结果追加到CPU2/CPU3共享Input；CPU3对外LTD Modbus直接暴露同一组共享地址和字段语义。

#### 共享语义与两端行为

- `DeviceParameters.sensorType`的地址、32位宽度、结构偏移和持久化位置保持不变，合法值新增15；CPU2枚举名为`MULTIPARAM_V4_SENSOR`，CPU3同步相同数值和“多参数V4”显示文本。
- CPU2将类型15接入V4主动64字节帧和8字节交互事务；CPU3只消费CPU2已确认的类型值，不解析传感器侧V4帧。收到未知类型时保留原始值并告警，批量参数同步不得把未知值反写CPU2。
- CPU2调试Input块在原`0x0100~0x011F`之后追加8个寄存器：`0x0120~0x0121`磁零点电压、`0x0122~0x0123`动力黏度、`0x0124~0x0125`运动黏度、`0x0126~0x0127`传感器供电电压。四项均为IEEE754 Float32、高16位在前，分别使用V、cP、cSt、V。
- CPU2从最近一次有效V4主动快照发布上述四项；初始化或尚无有效快照时字段为0。CPU3将调试轮询块末端同步扩展到`0x0128`，读取后写入本机共享快照。
- CPU3外部LTD FC03通过`CPU2_CommReadHoldingSnapshot()`读取类型15和协议版本32，FC04通过`CPU2_CommReadInputSnapshot()`直接读取上述四项；板间共享区与LTD对外共享区不是两套协议。快照未建立、CPU2断线、协议不匹配或刷新中时，外部请求返回异常`0x06`（Busy），不得回退为陈旧缓存。
- 协议31定义的Newhall扭力模块温度槽`0x0116~0x0117`、故障码、既有共享寄存器、命令码和状态码继续沿用；协议32只在尾部追加四项，不移动既有地址。

#### 兼容性和存储边界

- 协议32改变既有共享字段的合法值集合，扩大调试Input块并新增LTD对外可读字段，与协议31及更早程序不兼容；CPU2和CPU3必须严格使用相同`DEVICE_PROTOCOL_VERSION=32`，旧协议31组合由现有严格相等门禁拦截。
- 协议32首个正式固件组合为CPU2 `V1.37.0.0` / CPU3 `V1.37.0.0`，两端必须配套升级。
- 外部主机读取FC03 `0x0000~0x0001`可得到类型15，读取FC03 `0x000E~0x000F`可得到协议版本32；只有确认协议32后才应解释FC04 `0x0120~0x0127`。旧上位机若固定读取整个调试块，必须把长度从32个寄存器更新为40个寄存器。
- CPU2 `DEVICE_PARAM_VERSION`保持3；`DeviceParameters`结构大小、字段偏移、`struct_size`、magic、CRC范围、FRAM A/B地址和单槽容量均不变，升级不会恢复出厂或清除其它现场参数。
- 新固件允许持久化`sensorType=15`。降级前必须先把V4传感器参数65切回8字节交互方式，并把传感器类型改为旧固件可识别的值；旧固件不能可靠处理类型15或持续主动帧。

#### 验证边界

- 静态契约需覆盖双端协议32、类型15数值一致、CPU3显示与未知值保护、V4主动帧解析、交互事务、UART6接收恢复、参数65窗口仲裁，以及四项Input地址、块末端和Float32高字在前编码一致。
- LTD对外测试需分别读取FC03类型15、FC03协议版本32和FC04四项结果，并覆盖快照有效时的数值一致性及快照未建立/失效时的Busy响应。
- CPU2、CPU3按最终V1.37.0.0源码执行clean-first构建；协议32/31交叉组合的运行拦截、真实DM4 500 ms主动上报、RS485半双工时序、参数65持久化、OLED显示和长时间运行仍需目标板或台架验证。

### 协议版本 33

主题：把AO电流修正、最终目标和最近实发电流的固定点倍率由x100提升到x1000，使修正命令可表达0.001mA；基础AO电流配置仍保持x100。

#### 共享语义与两端行为

- `DeviceParameters.ao_output`第4个32位槽原位改名为`current_correction_mA_x1000`，默认0、范围-1000～+1000，单位0.001mA；LTD地址仍为`0x0606~0x0607`，LH地址仍为`0x0018~0x0019`，均按高字在前的Int32补码传输。
- `fixed_current_mA_x100`、`fault_current_mA_x100`、`power_on_current_mA_x100`和`simulation_current_mA_x100`等基础AO参数继续使用0.01mA。CPU2最终目标公式为`base_mA_x100 * 10 + correction_mA_x1000`，仅在AO启用态叠加一次并限制到3.200～24.000mA；禁用态固定3.400mA且不修正。
- `AoOutputRuntime.target_mA_x1000`和`last_sent_mA_x1000`发布0.001mA整数值。CPU3“电流修正”输入和“输出电流”只读显示均使用三位小数；最近实发值仍表示成功下发给AD5421的命令，不是物理环路实测值。
- AD5421新增x1000整数接口，DAC码换算使用64位整数四舍五入；旧x100接口保留为兼容包装。目标判重、失败重试和驱动恢复均比较x1000值，避免0.001～0.009mA变化在写入前被丢失。

#### 兼容性和存储边界

- 协议33与协议32的AO字段倍率及运行态解释不兼容，CPU2和CPU3必须严格使用相同`DEVICE_PROTOCOL_VERSION=33`；外部LTD/LH主机和上位机也必须按协议33解释修正值。
- CPU2加载协议26～32存储时，将旧`current_correction_mA_x100`数值乘10迁移为x1000；协议25及更早把复用槽归零，未知未来版本同样归零。范围外值归一化为0，避免错误修正。
- `DEVICE_PARAM_VERSION`保持3，13个AO槽、`DeviceParameters`大小、字段偏移、`struct_size`、magic、CRC范围、FRAM A/B地址和单槽容量均不变；本次迁移不应因结构变化恢复出厂参数。
- 协议32与33参数快照不得跨版本直接恢复，避免把旧x100原始值按x1000解释。协议33首个CPU2/CPU3正式固件组合为CPU2 V1.39.0.0 / CPU3 V1.38.0.0。

#### 验证边界

- 静态与主机测试应覆盖`-0.001mA -> FFFF FFFF`、`-0.250mA -> FFFF FF06`、`+1.000mA -> 0000 03E8`，以及基础x100、修正x1000、最终目标x1000的单次换算关系。
- CPU2/CPU3及上位机自动检查和构建不能证明真实电流环0.001mA精度，也不能替代RS485、FRAM掉电迁移、OLED显示、AD5421边界饱和和断环恢复台架验证。
