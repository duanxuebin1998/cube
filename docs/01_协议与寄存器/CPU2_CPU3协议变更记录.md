...
- 旧程序语义：保留字段，默认值为 `0`
- 当前正式固件组合：协议版本 `31`，CPU2 `V1.36.2.0` / CPU3 `V1.36.0.0`；协议31首个正式组合为双端`V1.36.0.0`
- 当前开发语义：协议版本 `31`；复用未使用的扭力参数槽传递Newhall扭力模块温度，CPU2/CPU3仍按协议版本严格相等判断
...
主题：在既有`sensorType`共享字段中增加`MULTIPARAM_V4_SENSOR=15`，使CPU2与CPU3能够一致识别和显示多参数V4传感器。
...
- 协议31定义的Newhall扭力模块温度槽`0x0116~0x0117`、故障码、其它共享寄存器、命令码和状态码继续沿用，不因协议32重新解释。

#### 兼容性和存储边界

- 协议32改变既有共享字段的合法值集合及CPU3消费语义，与协议31及更早程序不兼容；CPU2和CPU3必须严格使用相同`DEVICE_PROTOCOL_VERSION=32`，旧协议31组合由现有严格相等门禁拦截。
...
- 静态契约需覆盖双端协议32、类型15数值一致、CPU3显示与未知值保护、V4主动帧解析、交互事务、UART6接收恢复和参数65窗口仲裁。
