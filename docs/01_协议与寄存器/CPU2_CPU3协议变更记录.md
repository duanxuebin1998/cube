...
- 旧程序语义：保留字段，默认值为 `0`
- 当前开发及最新发布语义：协议版本 `24`，CPU2 `V1.29.0.0` / CPU3 `V1.27.0.0`
...
| 24 | 待提交阶段升版确定 | 待提交阶段升版确定 | 200 | 保持AO地址和结构尺寸不变；输出源1改为传感器位置，原上电/非跟随电流改为初始电流，运行态来源0改为初始并新增来源7保持；过程缓存只由成功写入AD5421的过程目标更新。 |

## 兼容判断规则

1. CPU2 启动时把本机 `DeviceParameters.protocolVersion` 修正为当前协议版本，并发布到保持寄存器。
2. CPU3 上电或通信恢复后，先单独读取稳定地址 `0x0010~0x0011` 的 CPU2 协议版本字段；协议尚未确认前不得访问当前协议新增的扩展寄存器。
