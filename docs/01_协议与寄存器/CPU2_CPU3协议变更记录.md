...
- 旧程序语义：保留字段，默认值为 `0`
- 当前开发及发布程序语义：协议版本 `21`；首个正式固件组合为 CPU2 `V1.27.0.0` / CPU3 `V1.25.0.0`
...
- CPU2 V1.25.0.0～V1.26.0.0移出当前枚举的10个通用码只保存在 `LTD故障代码统一表.xlsx` 的“历史故障代码_CPU2V1.25.0.0-V1.26.0.0”工作表，不与更早停用项合并统计，不得复用为新含义。
...
- AO当前菜单固定为5组、15个实际显示项：13项可操作，其中12项为持久化配置、`AO输出仿真`为非持久化运行期开关；`AO输入值`和`AO输入百分比`为2项只读运行态。`AO SIL/WHG`和`DAC回读电流`仅保留内部设计位置，当前不显示、不启用，不计入15项菜单。
