...
- 旧程序语义：保留字段，默认值为 `0`
- 当前工作区目标：协议版本 `35`；首个正式 CPU2/CPU3 固件组合待版本提交时确定
- 当前语义：统一DM4物理传感器类型为14，Safe作为DM4的预留运行协议模式；CPU2加载旧值15时迁移并写回14；CPU2/CPU3仍按协议版本严格相等判断
...
| 35 | 待发布 | 待发布 | 200 | 将同一物理DM4传感器统一为`sensorType=14`，V4为普通运行协议，Safe为预留运行协议模式；协议32～34持久化的旧值15由CPU2启动时迁移并写回14。字段地址、宽度、参数结构、`DEVICE_PARAM_VERSION=3`和CPU3本机参数版本`0x0007`不变。 |
