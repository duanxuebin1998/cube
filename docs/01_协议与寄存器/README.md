...
| CPU3 外部 COM 协议读取与切换 | `CPU3外部COM协议切换帧.md` | 提供 `0x46/0xFF` 当前协议只读查询、远程切换、独立响应、防本地回显误判、旧固件回退和单端口切换时序说明 |
...
| DM4传感器协议 | `传感器通信协议/DM4传感器协议/` | DM4 V4.0非安全通信协议、原始工作簿、有效参数、状态字、主动64字节帧、8字节交互事务和`D/L/M/S`控制定义；真实硬件联调仍待完成 |
| Newhall 称重主控协议 | `传感器通信协议/Newhall称重传感器协议/` | 归档独立 MSP430FR2476 称重主控的 `19200 8N1` UART 协议、`0x83` 全量数据帧、BCC、字段格式和联调边界；CUBE协议31已实现CPU2被动接收`0x83`重量/温度帧并由CPU3显示，协议32～34继续沿用，`0x81/0x82`、下行查询和真实硬件联调仍不在已验证范围 |
...
| 主题 | 当前结论 | 依据 |
| --- | --- | --- |
| 共享协议版本 | 当前正式协议为34，组合为CPU2 V1.40.0.0 / CPU3 V1.40.0.0，双端必须严格相等。协议34保持地址和参数结构不变，启用12-6并新增12-17编码器故障语义 | `LTD_MAIN_CPU2/Services/ParamStorage/system_parameter.h`、`LTD_DISPLAY_CPU3/Application/system_param/system_parameter.h`、`CPU2_CPU3协议变更记录.md` |
...
| LTD 对外协议 | 只维护一套标准Modbus协议，沿用地址宏/枚举和`HoldingRegisterArray[]`、`InputRegisterArray[]`直映射格式；CPU3读已确认快照，FC10等待CPU2合法ACK，七个命令前置参数以ACK确认本次值并后台刷新完整快照；协议32增加多参数V4类型，协议33把AO修正及目标/实发值改为x1000，协议34增加编码器故障语义，历史记录按CPU2程序版本选故障表 | `LTD共享Modbus协议/LTD共享Modbus协议卷.md` |
| CPU2 参数存储 | `DEVICE_PARAM_VERSION = 3`，当前版本头为`V1.40.0.0`；协议34不改变`DeviceParameters`结构、CRC范围或参数FRAM A/B槽，也不清现场参数。协议33 AO迁移、协议34编码轮周长0归一化和既有协议迁移规则继续生效 | `LTD_MAIN_CPU2/Services/ParamStorage/system_parameter.c`、`../00_构建与版本/CPU2参数存储升级清单.md` |
...
| AO当前语义 | 沿用协议24的初始/过程/保持状态和协议27固定功能块；协议33把单一电流修正及最终目标/实发值统一为x1000，协议34继续沿用。当前菜单为5组17项 | `CPU2_CPU3协议变更记录.md`、`系统参数出厂默认值.md`、`AO电流输出/4-20mA电流输出逻辑.md`、`../04_界面与菜单/CPU3当前屏幕菜单树.md` |
...
| SI Profile 独立兼容 | 共享协议14起支持独立SI Profile；协议18追加阶段、周期计数和有效点进度，并把CPU3本机参数版本升至`0x0007`；协议27重排CPU2/CPU3共享地址，协议28～34继续沿用SI对外地址、生命周期和CPU3本机FRAM布局 | `CPU2_CPU3协议变更记录.md`、`SI协议适配/02_协议映射/SI协议兼容映射表.md`、`../00_构建与版本/版本改动与测试/2026-07-15_CPU2_V1.26.0.0_CPU3_V1.24.0.0_SI协议18生命周期与现场兼容_改动与测试方案.md` |
| 故障码责任域和编号 | 协议29取消12-11并交换18/21类别，协议30新增23-1～23-6，协议34启用12-6并新增12-17。当前为135项CPU2共享故障、2项CPU3本机故障；其中使用中134项、保留3项。不提供旧码别名，历史表按CPU2程序版本查询 | `CPU2_CPU3协议变更记录.md`、`../00_构建与版本/故障码/README.md`、`../00_构建与版本/故障码/LTD故障代码统一表.xlsx` |
...
- 修改 LTD 外部功能码、寄存器窗口、快照/写确认或异常码行为时，同步更新 `LTD共享Modbus协议/`、协议事实清单和受影响的现有专项契约。
