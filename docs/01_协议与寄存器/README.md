...
| 分类 | 资料 | 用途 |
| --- | --- | --- |
| CPU2/CPU3 共享协议 | `CPU2_CPU3协议变更记录.md` | 记录 `DEVICE_PROTOCOL_VERSION`、共享寄存器、参数语义、兼容影响和验证结果；当前共享协议版本为 `17` |
...
| 主题 | 当前结论 | 依据 |
| --- | --- | --- |
| 共享协议版本 | `DEVICE_PROTOCOL_VERSION = 17`，CPU2/CPU3严格相等才兼容；协议16及更早版本与协议17的故障码编号不能混用 | `LTD_MAIN_CPU2/Services/ParamStorage/system_parameter.h`、`LTD_DISPLAY_CPU3/Application/system_param/system_parameter.h`、`CPU2_CPU3协议变更记录.md` |
| LTD 对外协议 | CPU3菜单中的LTD与CPU2/CPU3板间协议按同一套协议管理；CPU3读缓存快照，FC10等待CPU2合法ACK后回成功；32位故障码按协议17的统一表解释 | `LTD共享Modbus协议/LTD共享Modbus协议卷.md` |
...
| 故障码责任域和编号 | 共享协议17按二代计量仪责任域重新分类、删减并重编号：63项CPU2共享故障、1项CPU3本机故障；LTD、DSM和CPU2串口日志中的完整32位故障码均按协议17统一表解释，旧编号只用于对应历史记录 | `CPU2_CPU3协议变更记录.md`、`../00_构建与版本/故障码/README.md`、`../00_构建与版本/故障码/LTD故障代码统一表.xlsx` |
