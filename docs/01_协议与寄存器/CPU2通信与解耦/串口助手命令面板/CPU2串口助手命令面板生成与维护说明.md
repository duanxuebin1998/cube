...
- `STOP/HELP/VER?/STAT?/ERR?/PWR?/ENC?/PWRTEST` 必须保留在第一组。
...
生成脚本会校验固定顶层字段、第一组、字段完整性、发送属性、组内重复、八个系统/掉电调试命令及 `10` 组/`70` 按钮基线。新增或删除按钮时，应有意识地同步更新脚本中的规模断言。
...
3. 先执行 `VER?`、`STAT?`、`ERR?`、`PWR?`、`ENC?`，确认均收到 `ACK` 和对应查询内容。
