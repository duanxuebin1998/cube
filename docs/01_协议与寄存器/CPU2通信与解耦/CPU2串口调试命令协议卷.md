...
V4 调试命令在 `MeasureStart` 之前执行，不启动电机，也不把命令失败写入整机全局故障状态。除只读快照和诊断命令外，每次交互事务都会打印实际 TX/RX 长度、HEX、结果和校验阶段。当前 V4 发包仍遵循通信层的临时广播地址策略。
...
协议限制：P26-P64 不可读；P65-P159 虽为可配置区，但本命令族不开放任意写入，避免误改地址、存储周期和标定参数。模式、功能和通信方式只通过具有状态确认和恢复策略的专用命令修改。
