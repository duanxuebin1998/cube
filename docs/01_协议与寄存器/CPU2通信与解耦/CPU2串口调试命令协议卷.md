...
- 当前源码版本头为CPU2 `V1.27.0.0`，CPU2/CPU3开发共享协议版本为 `21`，CPU2参数存储版本为 `3`；本卷所述严格收帧和命令解析自CPU2 `V1.22.0.0`引入。CPU2 `V1.27.0.0`与CPU3 `V1.25.0.0`为协议21正式组合。
...
`ACK` 表示命令已通过解析并被接受，不表示电机动作、测量流程或长时间测试已经成功完成。最终结果仍以随后业务打印、设备状态和错误码为准。`ERR?` 和统一错误日志输出完整32位内部故障码；当前协议21固件沿用协议19建立的统一故障码表，协议18及更早版本的日志必须使用对应CPU2程序版本的历史编号表，不能只看十六进制值跨版本判断故障类别。
