...
本文同时维护需求、设计、实现状态、影响范围和验证口径。2026-07-01 本轮实现已将 SI profile 从普通分布测量解耦，并同步 CPU2/CPU3 参数、菜单、自动调度、报警合成和协议文档。
...
- Profile 模式按 SI 语义应先到罐底，第 0 点固定为罐底点；`40001 Profile First Point` 是 Point 0 之后的第一个停点。
- Manual 模式用于维护。下发 Up/Down/Stop 会停止 profile 或液位跟踪并进入 Manual；Manual 期间报警和液位更新口径需要按现场需求确认。
...
| profile 测量流程 | 复用普通分布测量流程，点阵由现有分布测量结果生成 | CPU2 新增 SI 独立流程，按底部 Point0 + 后续停点生成点阵 |
...
4. `40001 Profile First Point` 表示 Point 0 之后的第一个停点；Point 0 固定为底部点，Point 1 = First Point。
5. `40002 Profile Increment` 固定为从底部向上递增；`40003 Profile Dwell Time` 是每个点停稳后的等待时间。
...
7. 如果真实探底失败：有旧底部位置时使用旧底部位置继续 profile；没有旧底部位置时使用当前位置采集 Point0 并继续后续 profile 流程，但应记录失败状态并避免刷新底部参考。
8. 如果未达到探底频次且已有有效底部位置：不执行真实探底；使用上次探底保存的底部位置运行到底部测量位置。
9. 在底部测量位置等待稳定，采集 `Point 0` 的位置、温度、密度。
10. 按 CPU2 保存的 profile 参数继续生成后续停点：`first_point`、`increment`、`dwell_time`。
...
| 参数 | 含义 | 当前归一化范围 | 默认值 |
| --- | --- | --- | --- |
| `si_profile_first_point` | Point0 后首个停点，CPU2 内部单位 `0.1mm` | `0` 会归一化为默认值 | `1000` |
...
底部测量位置使用上次探底保存的位置。没有有效底部位置时应强制探底；若探底仍失败，则使用当前位置采集 Point0 并继续后续 profile 流程，但不能把失败位置写成新的底部参考，也不能清除“首次需探底”状态。该约束已加入 `tools/check_si_protocol_contract.py`，避免后续把 fallback 位置误写为可信底部。底部点偏移当前未确认，不建议把普通分布测量 `spreadBottomLimit` 直接当作 SI Point 0。
...
| --- | --- | --- | --- |
| `40001` | Profile First Point | 底部点 Point 0 之后的首个停点，Point 1 = First Point | mm，`0~65535` |
| `40002` | Profile Increment | 从底部向上的 profile 每点步距 | mm，`1~65535` |
...
| --- | --- | --- | --- | --- |
| `SI Profile` | `首点` | CPU2 `si_profile_first_point` / SI `40001` | mm，按手册默认值和范围 | Point0 固定为底部点，首点是 Point1 的位置；菜单写 CPU2 参数 |
...
5. 没有有效底部位置时必须强制探底；探底失败且仍无旧底部位置时使用当前位置采集 Point0 并继续 profile，但不得刷新底部参考。
6. 第 0 点必须写入 `30021~30023` 对应的第一组三元组。
7. `40001` 对应 Point 0 之后的首个停点，不能覆盖 Point 0。
8. profile 最多输出 200 点；profile 前不单独找液位，候选点运行中判定液面以上后停止，不采集该液面以上点；达到上限或点数上限也停止。
9. profile 失败时不置 `10005 Profile Complete`，也不递增完成计数。
