...
本文同时维护需求、设计、实现状态、影响范围和验证口径。2026-07-01 本轮实现已将 SI profile 从普通分布测量解耦，并同步 CPU2/CPU3 参数、菜单、自动调度、报警合成和协议文档。2026-07-02 根据官方中文全文译稿复核后，新增 Manual、Auto、Cal、Profile 生命周期和测量可信性偏差说明。
...
当前状态已经从“基础协议兼容版”推进到“SI profile 独立流程兼容版”。仍需现场复核的内容主要是 SI协议官方手册逐项默认值、真实 PLC 轮询节奏、密度 16 位范围、真实上下液位传感器硬件等。
...
- Profile 模式按 SI 语义应先到罐底采集第 0 点；对外点阵中 Point0 位置字段固定为 `0`，`40001 Profile First Point` 是 Point1 的绝对位置。
...
| --- | --- | --- |
| SI Profile 线圈 | 第一版 `00004` 写入后下发 `CMD_MEASURE_DISTRIBUTED` | 下发 `CMD_SI_PROFILE = 20`，并在 CPU3 锁存 profile 开始时间 |
| profile 测量流程 | 复用普通分布测量流程，点阵由现有分布测量结果生成 | CPU2 新增 SI 独立流程，按底部采样点 + 后续绝对停点生成点阵；Point0 对外位置字段固定为 `0` |
...
| profile 时间戳 | 第一版按完成计数变化锁存完成时刻 | CPU3 收到或触发 profile 命令时锁存测量开始时间 |
...
| Profile 时间戳 | `30007~30010` 是 CPU3 命令触发时间 | 不能作为 Point0 实际采集时间 |
| 探底失败 fallback | 无旧底且探底失败时可用当前位置继续 profile | 点位坐标和 Point0 物理可信性低于官方 SI 语义 |
...
| SI Modbus | `40001~40003` 写 CPU2 参数，`40010~40023` 写 CPU3 参数；`00004 Profile` 锁存 timestamp 并下发 `CMD_SI_PROFILE`；`si_modbus_periodic_task()` 落地自动调度 | `si_modbus_slave.c/h`、`app_main.c` |
...
4. `40001 Profile First Point` 表示 Point1 的绝对位置；Point0 在底部采样，但对外位置字段固定写 `0`。
...
17. `30007~30010 Profile Timestamp` 改为 profile 测量开始时间，口径为 CPU3 收到或触发 profile 命令的时间；手动 profile 和自动 profile 口径一致。
18. SI 探底频次默认值为 `1`；`1` 表示每次 profile 前探底，更大值表示每 N 次触发探底一次，计数按进入 CPU2 SI profile 入口的触发次数统计，不按成功完成次数统计。
19. 探底失败且没有旧底部位置时，使用当前位置作为底部采样运动目标继续；Point0 对外位置字段仍固定写 `0`。
20. `40001/40002/40003/40010~40023` 默认值按 SI协议手册默认值。
21. 自动 profile 打断当前测量时直接覆盖命令。
22. SI profile 成功锁存后，若当前没有其它待执行命令，CPU2 会排队 `CMD_FIND_OIL`，下一轮主循环自动找液位并回液位跟随；失败或命令切换时不自动找液位。
...
- `40010~40013` 自动 profile 和 `40014~40023` 报警限值继续由 CPU3 持久化；CPU3 不再通过命令上下文临时下发 profile 参数。
...
| Profile 开始时间戳、当前时间、SI 状态合成 | CPU3 | `30007~30010` 在 CPU3 收到或触发 profile 命令时锁存 |
...
| CPU3 SI协议参数 | `auto_profile_interval`、`auto_profile_enable`、`auto_profile_hour`、`auto_profile_minute`、`alarm_limit_*`、后续兼容开关和诊断项 | CPU3 | 对应 `40010~40023`，由 CPU3 掉电保存和合成报警/调度 |
...
3. 进入 SI profile 运行状态，清除上一轮 profile 完成锁存、点阵和报警状态。
4. 锁存 profile 测量开始事件，供 CPU3 输出 `30007~30010`。
5. 根据 SI 探底次数频次计数判断是否需要真实探底；计数按进入 CPU2 SI profile 入口的触发次数推进，上电后首次 profile 必须探底。
6. 如果达到探底频次或当前没有有效底部位置：先执行真实探底；探底成功后刷新内部底部参考或底部位置。
7. 如果真实探底失败：有旧底部位置时使用旧底部位置继续 profile；没有旧底部位置时使用当前位置作为底部采样运动目标并继续后续 profile 流程，但应记录失败状态并避免刷新底部参考；对外 Point0 位置字段仍写 `0`。
8. 如果未达到探底频次且已有有效底部位置：不执行真实探底；使用上次探底保存的底部位置运行到底部测量位置。
9. 在底部测量位置等待稳定，采集 `Point 0` 的温度、密度；写入点阵时 Point0 位置字段固定为 `0`，不写实际底部高度。
10. 按 CPU2 保存的 profile 参数继续生成后续停点：Point1 = `first_point`，Point2 = `first_point + increment`，PointN = `first_point + (N-1) * increment`。
11. 逐点运行、停稳后等待、采集温度和密度；候选点被判定为液面以上时停止且不写入该点，达到 200 点上限、上限位置或其它停止条件也停止。
12. 由 CPU3 扫描并生成能合成的 profile 报警状态；如后续发现必须依赖 CPU2 原始过程数据，再补充最小共享结果。
13. 写入 `g_measurement.density_distribution`，置位 `profile_complete_latched`，递增 `profile_complete_counter`。
14. 进入 SI profile 完成状态，并在没有其它待执行命令时排队 `CMD_FIND_OIL`。
...
底部测量位置使用上次探底保存的位置。没有有效底部位置时应强制探底；若探底仍失败，则使用当前位置作为底部采样运动目标并继续后续 profile 流程，但不能把失败位置写成新的底部参考，也不能清除“首次需探底”状态。该约束已加入 `tools/check_si_protocol_contract.py`，避免后续把 fallback 位置误写为可信底部。SI 对外点阵的 Point0 位置字段固定写 `0`，不使用普通分布测量 `spreadBottomLimit`，也不输出实际底部高度。
...
| --- | --- | --- | --- |
| `40001` | Profile First Point | Point1 绝对位置，不能覆盖 Point0 | mm，`0~65535` |
...
- `40010~40013`、`40014~40023` 由 CPU3 作为 SI 专用参数持久化；CPU3 本机参数 FRAM 版本升至 `0x0006`，提供 V3/V4/V5 迁移。
...
第一版由 CPU3 在收到手动 profile 命令或自动 profile 触发并准备下发 `CMD_SI_PROFILE` 的时刻锁存开始时间，并保存在 SI profile 运行态中。自动 profile 和手动 profile 使用同一锁存入口。
...
| Profile 完成后状态 | 成功锁存后先进入完成态，再排队 `CMD_FIND_OIL`，下一轮主循环回液位跟随 |
...
| `profile_complete_latched`、`profile_complete_counter` | profile 完成锁存和 CPU3 时间戳事件 |
...
6. CPU3 把 `00004 Profile` 改为下发 `CMD_SI_PROFILE`，并落地自动调度、状态合成、报警合成和 timestamp 锁存。
...
| 2. CPU3 协议参数 | 自动调度和报警限值存入 CPU3 | 新增 CPU3 SI 参数组，覆盖 `40010~40023`；CPU3 本机参数 FRAM 版本升至 `0x0006` | 无 | 自动调度和报警限值掉电保存 |
...
| 5. 启动命令 | Profile 只下发启动命令，不带上下文 | 下发 `CMD_SI_PROFILE` 前锁存 timestamp 和触发来源 | 接收 `CMD_SI_PROFILE` 后读取本机 SI profile 参数 | 共享协议版本和命令号检查 |
...
| 8. 状态和报警 | 对外状态符合 SI 语义 | 合成 `10002/10003/10010`，保持 `10001` 现有逻辑；按 `40014~40023` 合成当前值和 profile 报警 | 产生 profile complete、counter、点阵、真实失败状态和必要偏差原始结果 | 状态位、报警置位和恢复、timestamp 开始时间 |
...
| --- | --- | --- | --- | --- |
| `SI Profile` | `首点` | CPU2 `si_profile_first_point` / SI `40001` | mm，按手册默认值和范围 | Point0 在底部采样但对外位置字段固定为 `0`；首点是 Point1 的绝对位置；菜单写 CPU2 参数 |
...
3. `display_tankopera.c`：在“测量命令 -> 密度分布测量”菜单新增 `SI Profile` 无参命令项，确认后调用 SI profile 共享启动入口，锁存开始时间并下发 `CMD_SI_PROFILE`。
...
6. `cpu3_comm_display_params.h/c`：在 CPU3 本机参数结构中新增 `40010~40023` 对应 SI 参数组，补齐默认值、范围归一化、`Cpu3Local_ReadValue`、`Cpu3Local_WriteValue` 和 FRAM 镜像迁移；自动调度相关参数写入后重新计算下一次触发时间。
7. `si_modbus_slave.c`：保持寄存器读写按地址分流，`40001~40003` 访问 CPU2 profile 参数，`40010~40023` 访问 CPU3 SI 参数；非法值返回 SI 异常码，合法值应与菜单回读一致。
8. CPU3 调度模块：提供 `Si_Profile_Request(reason, overwrite)` 或等价入口，供 Modbus `00004 Profile`、自动调度和菜单触发共用；入口负责锁存 profile timestamp、发送 CPU2 启动命令。
...
| 掉电保存 | 修改 `40001~40003` 后 CPU2 重启保持；修改 `40010~40023` 后 CPU3 重启保持；菜单和保持寄存器均保持新值 |
...
| `40010~40023` 独立保持寄存器 | 否 | CPU3 参数存储 | CPU3 持久化自动调度和报警限值，CPU3 本机参数 FRAM 版本 `0x0005 -> 0x0006` |
...
| SI profile 隔离 | 普通分布和 Wärtsilä 分布完成后不置 SI `10005`，`30006` 和点阵仍保持 SI 无效 |
...
6. 第 0 点必须写入 `30021~30023` 对应的第一组三元组，其中位置字段固定为 `0`，只有温度和密度来自实际底部采样。
...
10. profile 失败时不置 `10005 Profile Complete`，也不递增完成计数。
