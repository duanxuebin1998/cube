...
协议18的首个正式匹配组合为CPU2 `V1.26.0.0` / CPU3 `V1.24.0.0`，CPU3 FRAM使用V7。最新协议29正式组合为CPU2 `V1.34.0.0` / CPU3 `V1.34.0.0`；协议29不改变SI对外地址、生命周期或CPU3 FRAM布局，沿用协议27的共享地址重排和SI Stop取消当前测量语义。未修复风险见 `../01_计划与需求/SI协议待确认与后续清单.md` 第7节。
