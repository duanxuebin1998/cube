...
| 项目 | 当前结论 |
| --- | --- |
| 协议契约 | CPU2/CPU3 当前开发基线为 `DEVICE_PROTOCOL_VERSION = 18`；在协议17尾部追加SI Profile阶段、周期计数和有效点进度，不移动既有点阵和其它共享地址。协议18必须双端严格配套 |
...
当前CPU2/CPU3源码使用共享协议18，CPU3 FRAM使用V7；首个正式匹配组合为CPU2 `V1.26.0.0` / CPU3 `V1.24.0.0`。协议17及更早版本不得混搭；未修复风险见`../01_计划与需求/SI协议待确认与后续清单.md`第7节。
