...
| Point0 和点阵 | Point0 在底部采样，但对外位置字段固定写 `0`；`40001` 表示 Point1 的绝对位置；profile 前不单独找液位，运行中遇到液面以上点停止，且不写入该点 |
| 探底失败 fallback | 需要探底但失败时，如有旧底部位置则沿用旧底部；没有旧底部位置时使用当前位置完成底部采样，Point0 对外位置仍写 `0`，且不刷新底部参考 |
...
| Profile/Cal 完成后回 Auto | Profile 或 Cal 完成后返回 Auto 并跟踪液位 | SI profile 成功后先锁存完成态，再由 CPU2 排队 `CMD_FIND_OIL` 回液位跟随；`00002 Calibrate` 当前仍桥接 `CMD_CALIBRATE_ZERO`，不是完整 Level Calibration | Profile 后需等待自动找液位完成再把 `30004` 当当前液位；Cal 完整语义仍待确认 |
...
| Profile Timestamp | 更接近首个 profile 点采集时间 | `30007~30010` 为 CPU3 收到或触发 profile 命令时锁存 | 只能作为 profile 启动请求时间，不能作为 Point0 实采时间 |
| 无底部参考时的 profile | 应通过 Cal/Profile 重新建立 bottom reference，保证罐底基准可信 | 探底失败且无旧底时使用当前位置继续 profile，Point0 对外仍为 0，不刷新底部参考 | 数据可用性优先于官方物理基准；现场若要求严格一致，应改为失败而不是完成 |
...
两类字段的写入门禁不同：`40001~40003` 必须在状态、完整参数和当前连接协议快照均已建立、共享协议版本相等、参数刷新已完成且通信故障未锁存时写 CPU2，只有 CPU2 ACK 成功后才更新 `g_deviceParams` 和 SI 保持寄存器影子；失败返回设备忙 `0x06` 并保留原值。已发起的参数写若超时、响应非法或 ACK 丢失，内部参数快照会立即失效并强制补读四组，避免同轮其它普通命令在结果不确定时继续下发。`40010~40023` 只写 CPU3 本机参数和 FRAM，合法写入不依赖 CPU2 在线状态，不得被 CPU2 通信门禁误拦截。

官方语义补充：profile 启动后先到罐底采集 Point 0；`40001 Profile First Point` 是 Point1 的绝对位置，`40002 Profile Increment` 是从该位置开始向液面方向逐点增加的步距，`40003 Profile Dwell Time` 是每点停稳后的等待秒数。CPU2 不在 profile 前单独执行 `SearchOilLevel()`，运动候选点按实际底部、绝对首点、步距、罐高和 200 点上限生成；Point0 对外位置固定写 `0`，Point1 对外位置固定等于 `40001`，Point2 等于 `40001 + 40002`，PointN 等于 `40001 + (N-1) * 40002`。即使实际底部位置高于 `40001`，也不跳过 `40001`，仍运行到第一个可编程测点。逐点采样时若当前候选点被判定为液面以上点，则停止本轮 profile 且不把该点写入 profile 点阵。profile 成功锁存后，CPU2 会在没有其它待执行命令时排队 `CMD_FIND_OIL`，下一轮主循环恢复液位跟随。自动 profile 首次在 `40012/40013` 指定时刻运行，之后按 `40010` 分钟周期重复；如果只需要每天一次，官方建议 `40010=1440`。到点下发失败时不记录本分钟成功状态，CPU3 按 5 s 门限节流重试；只有下发成功后才对本分钟去重。关闭自动 profile 按官方口径使用 `40011=0`，当前实现也按此处理，并额外拒绝写入 `40010=0`。

| SI 地址 | 文档含义 | 当前映射字段 | 当前状态 | 备注 |
| --- | --- | --- | --- | --- |
| `40001` | Profile First Point | `g_deviceParams.si_profile_first_point` | 已接 | 协议单位 mm，CPU2 存储单位 `0.1mm`；Point0 对外位置写 `0`，40001 是 Point1 绝对位置 |
...
| `FC06 40001~40003` | 依赖 CPU2 参数 ACK | 设备忙 `0x06` | ACK 成功后才更新 `g_deviceParams` 和 `s_holding_regs` |
...
6. 已优化 `FC06` 基础值域检查和 profile 开始时间戳锁存；`40001~40003` 只在 CPU2 ACK 成功后提交影子，`40010~40023` 作为 CPU3 本机持久化参数不依赖 CPU2。
7. 已把“探底失败且无旧底部位置”收敛为一次性 fallback：使用当前位置作为底部采样运动目标并继续本轮 profile，但不刷新 SI 内部底部位置、不置底部位置有效、不清首次探底状态；Point0 对外位置字段仍固定为 `0`。该约束由 `tools/check_si_protocol_contract.py` 静态检查覆盖。
...
9. 已补齐 SI profile 成功后的 Auto 生命周期：成功锁存后若无其它待执行命令，CPU2 自动排队 `CMD_FIND_OIL`，下一轮主循环找液位并进入液位跟随；该约束由 `tools/check_si_protocol_contract.py` 静态检查覆盖。
...
| Profile 参数 | 写入 `40001~40003` 后触发 `00004 Profile` | CPU2 使用 SI 独立 profile 参数执行；Point0 对外位置字段为 `0`，Point1 从绝对位置 `40001` 开始，Point2 为 `40001+40002` |
...
| Profile 数据 | 读取 `10005`、`30006`、`30007~30010` 和 `30021~30620` | 未完成时点数和点阵为 0；完成后点数与有效点一致，点数不超过 200；时间戳为测量开始时间；随后应自动进入找液位流程 |
| 探底 fallback | 构造探底失败、有旧底部和无旧底部两类场景 | 有旧底部时沿用旧底部；无旧底部时使用当前位置做底部采样目标，Point0 对外位置仍为 `0`，但不刷新底部位置有效状态 |
...
| CPU3 本机参数离线写 | CPU2 离线时写 `40010~40023` 合法值 | 不受 CPU2 门禁影响，正常回显并按 CPU3 FRAM 口径保存 |
