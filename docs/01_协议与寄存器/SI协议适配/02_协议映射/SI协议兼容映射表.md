...
| 项目 | 当前结论 |
| --- | --- |
| 协议契约 | 最新正式组合为 `DEVICE_PROTOCOL_VERSION = 31`、CPU2 V1.36.2.0 / CPU3 V1.36.0.0。SI对外地址和生命周期仍沿用协议18；协议31不改变SI外部地址或字段语义，双端仍须按共享协议严格配套 |
...
协议18的首个正式匹配组合为CPU2 `V1.26.0.0` / CPU3 `V1.24.0.0`，CPU3 FRAM使用V7。当前协议31正式组合为CPU2 `V1.36.2.0` / CPU3 `V1.36.0.0`；协议31不改变SI对外地址、生命周期或CPU3 FRAM布局，沿用协议27的共享地址重排和SI Stop取消当前测量语义。未修复风险见 `../01_计划与需求/SI协议待确认与后续清单.md` 第7节。
