...
| 项目 | 当前结论 |
| --- | --- |
| 协议契约 | 当前整机开发及最新正式组合为 `DEVICE_PROTOCOL_VERSION = 24`、CPU2 V1.29.0.0 / CPU3 V1.27.0.0；SI地址和生命周期仍沿用协议18。协议19故障码、协议20 AO、协议21固定点代际和协议22故障码重排不改变SI契约；协议23 AO语义收敛和协议24 AO过程保持同样不移动既有SI地址；双端仍须严格配套 |
...
协议18的首个正式匹配组合为CPU2 `V1.26.0.0` / CPU3 `V1.24.0.0`，CPU3 FRAM使用V7。最新协议24正式组合为CPU2 `V1.29.0.0` / CPU3 `V1.27.0.0`；协议22只重排共享故障编号，协议23收敛AO语义，协议24增加AO过程保持，均不改变SI契约。协议19至24均未改变SI地址、生命周期和FRAM布局。未修复风险见 `../01_计划与需求/SI协议待确认与后续清单.md` 第7节。
