...
| 项目 | 当前结论 |
| --- | --- |
| 协议契约 | 最新正式组合为 `DEVICE_PROTOCOL_VERSION = 27`、CPU2 V1.31.0.0 / CPU3 V1.31.0.0。SI对外地址和生命周期仍沿用协议18；协议27重排CPU2/CPU3共享状态地址，并把SI Stop映射为取消测量命令16，双端仍须严格配套 |
...
协议18的首个正式匹配组合为CPU2 `V1.26.0.0` / CPU3 `V1.24.0.0`，CPU3 FRAM使用V7。最新协议27正式组合为CPU2 `V1.31.0.0` / CPU3 `V1.31.0.0`；协议27重排CPU2/CPU3共享地址，不改变SI对外地址、生命周期或CPU3 FRAM布局，但SI Stop改为取消当前测量。未修复风险见 `../01_计划与需求/SI协议待确认与后续清单.md` 第7节。
