...
| 项目 | 当前结论 |
| --- | --- |
| 协议契约 | 当前整机开发及最新正式组合为 `DEVICE_PROTOCOL_VERSION = 25`、CPU2 V1.30.0.0 / CPU3 V1.29.0.0；SI地址和生命周期仍沿用协议18。协议19故障码、协议20 AO和协议21固定点代际均不改变SI契约；协议22故障码重排不改变SI契约，协议23/24 AO语义和协议25水位预留参数也不改变SI契约；双端仍须严格配套 |
...
- SI协议官方手册逐项默认值仍需现场复核；当前代码采用 CPU2 首点 500mm、步距 1000mm、停留 10s、探底频次 1，CPU3 自动周期 60min、默认关闭。报警阈值 `0` 是有效值，不能再作为禁用值。
...
协议18的首个正式匹配组合为CPU2 `V1.26.0.0` / CPU3 `V1.24.0.0`，CPU3 FRAM使用V7。最新协议25正式组合为CPU2 `V1.30.0.0` / CPU3 `V1.29.0.0`；协议22只重排共享故障编号，协议23收敛AO语义，协议24增加AO过程保持，协议25增加水位预留参数，均不改变SI契约。协议19至25均未改变SI地址、生命周期和FRAM布局。未修复风险见 `../01_计划与需求/SI协议待确认与后续清单.md` 第7节。
