...
阅读口径：第 1～14 节保留 2026-05 第一版适配过程；第 15 节记录协议 14 引入 SI Profile 独立兼容的结果，第 16 节记录 2026-07-10 CPU2 通信门禁和写失败反馈。当前共享协议已升至 `DEVICE_PROTOCOL_VERSION = 15`，本次只调整设备内部故障码的责任域和编号，SI 地址、命令、状态、报警和 Profile 数据格式仍沿用协议 14 的既有实现。涉及当前地址归属、时间戳、RTC、状态或失败响应时，以第 15～16 节和当前源码为准。
