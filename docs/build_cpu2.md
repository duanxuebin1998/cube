# LTD_MAIN_CPU2 构建说明（GCC + CMake + Ninja）

## 前置依赖
- `cmake` (>= 3.20)
- `ninja`
<<<<<<< ours
- Arm GNU Toolchain（推荐，且可执行文件需在 `PATH` 中）：
  - `arm-none-eabi-gcc`
  - `arm-none-eabi-objcopy`
  - `arm-none-eabi-size`

> 说明：`cmake/toolchain-arm-none-eabi.cmake` 会优先使用 `arm-none-eabi-gcc`；若环境缺少该工具链，会退化为 `clang` 的 `arm-none-eabi` 目标尝试构建（用于尽量保持配置可运行）。
=======
- Arm GNU Toolchain（可执行文件需在 `PATH` 中）：
  - `arm-none-eabi-gcc`
  - `arm-none-eabi-g++`
  - `arm-none-eabi-objcopy`
  - `arm-none-eabi-size`

> 说明：该工程按 GCC 工具链构建，不再回退到 clang。
>>>>>>> theirs

## 手动构建
在仓库根目录执行：

```bash
cmake -S LTD_MAIN_CPU2 -B build/LTD_MAIN_CPU2 -G Ninja \
  -DCMAKE_TOOLCHAIN_FILE=cmake/toolchain-arm-none-eabi.cmake \
  -DCMAKE_BUILD_TYPE=Debug

cmake --build build/LTD_MAIN_CPU2
```

<<<<<<< ours
=======
## 源码收集范围
`LTD_MAIN_CPU2/CMakeLists.txt` 当前会编译：
- `Core/Src/*.c`
- `Application/Src/*.c`
- `Services/**/*.c`
- `Drivers/STM32F4xx_HAL_Driver/Src/*.c`
- `Drivers/Peripherals/src/*.c`（补齐 TMC5130 与 FRAM 驱动符号，解决 `stpr_* / stepper / WriteMultiData / ReadMultiData` 链接错误）
- `Core/Startup/startup_stm32f429zgtx.s`

>>>>>>> theirs
## 产物
构建完成后，`build/LTD_MAIN_CPU2/` 下会生成：
- `LTD_MAIN_CPU2.elf`
- `LTD_MAIN_CPU2.hex`
- `LTD_MAIN_CPU2.bin`
- `LTD_MAIN_CPU2.map`

## 一键构建脚本
可直接运行：

```bash
./tools/build_cpu2.sh
```
