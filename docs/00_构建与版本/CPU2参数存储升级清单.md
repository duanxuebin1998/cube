...
| CPU2 `V1.12.0.0` 到 `V1.12.2.0` 之间升级 | 通常保留 | 确认 FRAM 未损坏即可；`V1.12.1.2`/`V1.12.1.3` 新默认值只在恢复出厂或 FRAM 无效时生效，`V1.12.2.0` 不改变参数存储结构 |
...
| CPU2 `V1.12.0.1` 到当前 `V1.12.2.0` | 3 | 版本 3 内通常不清 | 当前未改变参数存储结构；`V1.12.1.2` 和 `V1.12.1.3` 仅调整恢复出厂默认值，`V1.12.2.0` 调整读取部件参数和调试数据语义 |
