...
| CPU2 `V1.12.0.0` 到 `V1.24.0.0` 之间升级 | 通常保留 | 确认 FRAM 未损坏即可；`V1.12.1.2`/`V1.12.1.3` 新默认值只在恢复出厂或 FRAM 无效时生效；`V1.20.2.0` 至 `V1.24.0.0` 等后续版本不改变 CPU2 参数存储结构。若从 `V1.24.0.0` 降级到不识别 `SAFE_SENSOR=14` 的旧固件，先恢复旧固件支持的传感器类型 |
...
| CPU2 `V1.12.0.1` 到当前版本头 `V1.30.0.0` | 3 | 版本 3 内通常不清 | 当前未改变参数存储结构总尺寸；期间新增字段优先复用原槽位或追加运行态，`DEVICE_PARAM_VERSION`、`DeviceParameters`大小、元信息和CRC范围保持不变 |
