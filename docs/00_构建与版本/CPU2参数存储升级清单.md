...
| CPU2 `V1.12.0.0` 到 `V1.36.2.0` 之间升级 | 通常保留 | 确认 FRAM 未损坏即可；`V1.12.1.2`/`V1.12.1.3` 新默认值只在恢复出厂或 FRAM 无效时生效；后续版本继续保持 `DEVICE_PARAM_VERSION=3` 和结构尺寸。协议25及更早升级到协议26及以后时，原SIL/WHG预留槽归零后解释为有符号AO电流修正。若从 `V1.24.0.0` 及以后降级到不识别 `SAFE_SENSOR=14` 的旧固件，先恢复旧固件支持的传感器类型 |
...
| CPU2 `V1.12.0.1` 到当前版本头 `V1.36.2.0` | 3 | 版本 3 内通常不清 | 当前未改变参数存储结构总尺寸；期间新增字段优先复用原槽位或追加运行态，`DEVICE_PARAM_VERSION`、`DeviceParameters`大小、元信息和CRC范围保持不变 |
...
1. `protocolVersion<26`或高于当前协议31时，`current_correction_mA_x100`强制归零；协议26～31合法值按有符号0.01mA解释。
