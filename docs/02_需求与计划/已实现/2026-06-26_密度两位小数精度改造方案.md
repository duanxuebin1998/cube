# 密度两位小数精度改造方案

日期：2026-06-26

状态：已落地代码改造，并已补充 CPU2 V1.20.0.0 / CPU3 V1.18.0.0 发布版本号、CHANGELOG 和版本测试记录。

## 1. 背景

当前项目中密度相关数据主要按 `kg/m3 x10` 维护，即显示 1 位小数。用户希望将密度精度改为 2 位小数。该需求表面上是显示小数位调整，但现有源码中密度值同时参与 CPU2 测量结果、CPU2/CPU3 内部 Modbus 共享寄存器、CPU3 状态页显示、CPU3 参数菜单、DSM/Wartsila/SI协议适配、液位密度跟随和密度修正参数。

因此本需求不能只修改 CPU3 显示层的小数位。如果只把显示 `points` 从 `1` 改为 `2`，现有 `x10` 原始值会被当成 `x100` 显示，例如原始值 `8321` 当前表示 `832.1 kg/m3`，显示小数位直接改成 2 后会错误显示为 `83.21 kg/m3`。

## 2. 目标

1. 将内部密度原始值从 `kg/m3 x10` 统一调整为 `kg/m3 x100`。
2. CPU3 状态页和参数菜单按 2 位小数显示密度。
3. CPU2/CPU3 共享寄存器语义保持一致，避免 CPU2 输出 `x100`、CPU3 仍按 `x10` 消费。
4. 对外协议保持既有口径，除 LTD 自有协议外，不因为内部精度升级改变 DSM、Wartsila 或 SI 等外部主站看到的倍率。
5. 明确参数存储、协议版本、固件版本、文档和测试影响，作为后续实现依据。

## 3. 已确认决策

| 序号 | 决策项 | 已确认口径 |
| --- | --- | --- |
| 1 | 改造范围 | 整体改造；内部密度结果、CPU3 显示、CPU3 参数菜单、CPU2/CPU3 内部 Modbus 和 LTD 自有协议统一支持两位精度 |
| 2 | 对外协议总体原则 | 不影响既有对外协议；除 LTD 自有协议外，外部主站看到的密度倍率保持原口径 |
| 3 | DSM 外部协议 | 保持现有对外倍率，不随内部 `x100` 改变；CPU3 在 DSM 输出边界做转换 |
| 4 | Wartsila 外部协议 | 保持现有 `scale = 10` / `x10` 口径；CPU3 在 Wartsila 输出边界把内部 `x100` 转回 `x10` |
| 5 | SI协议 | 保持现有 SI协议对外口径；当前 SI 已是协议侧 `x100`，内部升级后只取消多余 `*10`，对 PLC 表现不变 |
| 6 | CPU3 状态页显示 | 保留现有尾零裁剪逻辑；例如 `832.10` 可显示为 `832.1`，`832.12` 显示为 `832.12` |
| 7 | 密度修正参数 | 支持两位精度；旧基准 `10000` 迁移到新 `100000` 口径 |
| 8 | 旧 FRAM 参数 | 采用迁移保留策略，不默认恢复出厂参数 |
| 9 | 密度跟随阈值 | `oilLevelThreshold` 和 `oilLevelHysteresisThreshold` 一起改为两位精度口径 |
| 10 | CPU2/CPU3 成对升级 | 接受新旧 CPU2/CPU3 不兼容；通过升级 `DEVICE_PROTOCOL_VERSION` 阻止混搭 |

## 4. 当前源码口径

### 4.1 CPU2 密度转换宏

位置：`LTD_MAIN_CPU2/Services/ParamStorage/system_parameter.h`

当前关键宏：

```c
#define DENSITY_TO_RAW(d) ((uint32_t)((d) * 10.0f))
#define RAW_TO_DENSITY(raw) ((raw) / 10.0f)
```

含义：

| 工程值 | 当前 raw | 当前显示 |
| --- | --- | --- |
| `832.10 kg/m3` | `8321` | `832.1` |
| `832.12 kg/m3` | `8321` 或 `8322`，取决于浮点截断/四舍五入位置 | 只能体现 1 位 |

当前 `DENSITY_TO_RAW` 没有显式四舍五入，直接强转 `uint32_t`，存在向下截断风险。改造时建议一并改为四舍五入口径。

### 4.2 CPU2 密度结果来源

密度测量结果主要通过 `DENSITY_TO_RAW()` 写入：

| 场景 | 位置 | 当前行为 |
| --- | --- | --- |
| 单点测量/单点监测 | `LTD_MAIN_CPU2/Application/Src/measure_density.c` 的 `SinglePoint_ReadSensor()` | 把传感器 float 密度转成 raw 后写入 `DensityMeasurement` |
| 普通分布测量 | `LTD_MAIN_CPU2/Application/Src/measure_density.c` | 逐点调用单点读数，平均 raw 值 |
| 瓦锡兰区间密度 | `LTD_MAIN_CPU2/Application/Src/wartsila_density_measurement.c` | 点数据通过 `DENSITY_TO_RAW()` 写入 |
| 读取部件参数/调试缓存 | `LTD_MAIN_CPU2/Services/Sensor/sensor.c` | 把当前密度写入 `single_point_monitoring` 和 `single_point_measurement` |

CPU2 内部平均密度逻辑是对 raw 值求平均；raw 从 `x10` 改为 `x100` 后，平均逻辑本身仍成立。

### 4.3 CPU2/CPU3 共享寄存器

位置：

- CPU2 发布：`LTD_MAIN_CPU2/Services/Modbus/dataanalysis_modbus.c`
- CPU3 读取：`LTD_DISPLAY_CPU3/Communication/internal/main_board_modbus/dataanalysis_modbus.c`
- 共享枚举：`stateformodbus.h`

当前密度字段通过 32 位寄存器直接传输 raw 值：

| 字段 | 当前语义 |
| --- | --- |
| `REG_SINGLE_POINT_MEAS_DENSITY` | 单点测量密度，`kg/m3 x10` |
| `REG_SINGLE_POINT_MEAS_STD_DENSITY` | 单点测量标准密度，`kg/m3 x10` |
| `REG_SINGLE_POINT_MEAS_WEIGHT_DENSITY` | 单点测量计重密度，`kg/m3 x10` |
| `REG_SINGLE_POINT_MON_DENSITY` | 单点监测密度，`kg/m3 x10` |
| `REG_SINGLE_POINT_MON_STD_DENSITY` | 单点监测标准密度，`kg/m3 x10` |
| `REG_SINGLE_POINT_MON_WEIGHT_DENSITY` | 单点监测计重密度，`kg/m3 x10` |
| `REG_DENSITY_DIST_AVG_DENSITY` | 分布平均密度，`kg/m3 x10` |
| `REG_DENSITY_DIST_AVG_STD_DENSITY` | 分布平均标准密度，`kg/m3 x10` |
| `REG_DENSITY_DIST_AVG_WEIGHT_DENSITY` | 分布平均计重密度，`kg/m3 x10` |
| `REG_DENSITY_POINT_DENSITY(i)` | 分布点密度，`kg/m3 x10` |
| `REG_DENSITY_POINT_STD_DENSITY(i)` | 分布点标准密度，`kg/m3 x10` |
| `REG_DENSITY_POINT_WEIGHT_DENSITY(i)` | 分布点计重密度，`kg/m3 x10` |

这些字段地址和长度可以保持不变，但单位倍率从 `x10` 改为 `x100` 属于 CPU2/CPU3 共享协议语义变化，需要升级 `DEVICE_PROTOCOL_VERSION`。

### 4.4 CPU3 状态页显示

位置：`LTD_DISPLAY_CPU3/Application/display/display.c`

CPU3 状态页当前直接把密度 raw 传给 `OledValueDisplay()`，并传入小数位 `1`。

关键位置：

- 增量刷新路径：`Display_AddCurrentPageValueStatusSlots()` 中密度槽位使用 `1U`。
- 整页绘制路径：`oled_equipment()` 中密度显示使用 `1`。

`OledValueDisplay(value, ..., points, unit)` 会按 `points` 插入小数点，并裁剪末尾 0。因此只有 raw 数据改为 `x100` 后，状态页小数位才能安全改为 `2`。

### 4.5 CPU3 参数菜单

位置：`LTD_DISPLAY_CPU3/Application/system_param/system_parameter.c`

当前密度相关参数元数据：

| 参数 | 当前小数位 | 当前含义 |
| --- | --- | --- |
| `液位跟随密度` / `oilLevelDensity` | `1` | `kg/m3 x10` |
| `磁通量D` / `densityCorrection` | `1` | 显示为密度修正量 |
| `密度手输值` / `screen input D` | `1` | CPU3 本地手输密度值 |

如果这几个参数改为 2 位，需要同步 CPU2 参数解释和旧 FRAM 迁移策略。

### 4.6 CPU2 密度跟随和密度修正

位置：

- `LTD_MAIN_CPU2/Application/Src/measure_oilLevel.c`
- `LTD_MAIN_CPU2/Services/Sensor/sensor.c`

当前逻辑：

| 参数 | 当前解释 |
| --- | --- |
| `oilLevelDensity` | 通过 `RAW_TO_DENSITY(g_deviceParams.oilLevelDensity)` 转换为目标密度 |
| `oilLevelThreshold` | 密度跟随死区使用 `RAW_TO_DENSITY(raw_threshold)` |
| `oilLevelHysteresisThreshold` | 密度跟随滞后死区使用 `RAW_TO_DENSITY(raw_threshold)` |
| `densityCorrection` | 通过 `(densityCorrection - 10000) / 10.0f` 作为密度修正量 |

注意：`oilLevelThreshold` 和 `oilLevelHysteresisThreshold` 当前也复用 `RAW_TO_DENSITY()`。如果把宏整体改为 `/100`，这两个阈值的历史参数语义也会变化，需要一起纳入兼容策略。

### 4.7 外部协议口径

| 外部协议 | 当前处理 | 改造风险 |
| --- | --- | --- |
| SI | `si_density_raw_to_si_u16()` 当前把 CPU2 raw `x10` 转为 SI `x100`，即 `raw * 10` | 已确认对 PLC 口径保持不变；内部改为 `x100` 后取消额外 `*10` |
| Wartsila | 寄存器说明为 `scale = 10`，当前直接透传内部 raw | 已确认继续保持 `x10`，输出前需从内部 `x100` 转回 `x10` |
| DSM | 多个输入寄存器直接写 `g_measurement` 密度 raw | 已确认保持既有对外倍率，输出前按旧协议口径转换 |
| HART | CPU2 `hart.c` 通过 `RAW_TO_DENSITY()` 转工程值 | 宏改造后 HART 工程值路径可自然适配，但需要回归 |
| LTD 自有协议 | 需结合现有 LTD 协议映射确认具体入口 | 已确认可跟随内部两位精度改造，作为本次对外协议中的例外 |

## 5. 设计原则

1. 内部统一，边界转换：CPU2/CPU3 内部共享数据统一升级为 `x100`，DSM、Wartsila、SI 等外部协议在 CPU3 或对应协议边界转换到原协议要求的倍率。
2. 地址少变，语义升版：尽量不改变寄存器地址、结构体字段和 32 位长度，只改变密度字段倍率；通过 `DEVICE_PROTOCOL_VERSION` 表达不兼容语义变化。
3. 参数迁移明确：历史 FRAM 中 `x10` 密度参数不能直接按 `x100` 解释，必须迁移或恢复默认。
4. 显示跟随数据口径：显示小数位只在数据 raw 升级后调整，不做单独显示补偿。
5. 保留无效值：`UNVALID_DENSITY = 0` 不变，避免破坏现有无效值判断。
6. 对外兼容优先：除 LTD 自有协议外，对外协议寄存器地址、倍率和主站解析口径保持不变。

## 6. 总体方案

推荐采用“内部密度 raw 统一升级到 `kg/m3 x100`，外部协议边界按需降级或保留协议倍率”的方案。

改造后口径：

| 层级 | 改造后倍率 | 说明 |
| --- | --- | --- |
| CPU2 测量结果 | `kg/m3 x100` | 单点、单点监测、分布平均、分布点统一 |
| CPU2/CPU3 内部 Modbus | `kg/m3 x100` | 地址和长度尽量不变，协议版本升级 |
| CPU3 状态页 | `2` 位小数 | 直接显示 `x100` raw |
| CPU3 参数菜单密度项 | `2` 位小数 | 参数同步到 CPU2 时保存 `x100` raw |
| DSM | 保持现有对外倍率 | CPU3 在输出边界转换，不改变旧上位机解析口径 |
| Wartsila | 保持现有 `x10` | 输出前从内部 `x100` 转为 `x10`，建议四舍五入 |
| SI | 保持现有协议侧 `x100` | 内部已是 `x100`，只钳位不再额外乘 10，对 PLC 口径不变 |
| LTD 自有协议 | 跟随两位精度 | 作为外部协议例外，按 `x100` 同步更新映射和文档 |

## 7. 详细实施计划

### 阶段 0：需求确认

以下事项已确认：

1. 整体改造内部密度、CPU3 显示、CPU3 参数菜单、内部 Modbus 和 LTD 自有协议。
2. 除 LTD 自有协议外，DSM、Wartsila、SI 等外部协议保持现有对外口径。
3. DSM 保持现有对外倍率。
4. Wartsila 保持现有 `scale = 10`。
5. CPU3 状态页保留尾零裁剪。
6. `densityCorrection` 支持两位。
7. 旧 FRAM 参数采用迁移保留。
8. 密度跟随阈值一起改为两位。
9. 接受 CPU2/CPU3 必须成对升级。

后续实现只需补充确认 LTD 自有协议的具体入口和地址表；不再把 DSM/Wartsila/SI 是否改倍率作为开放问题。

### 阶段 1：CPU2 密度基础倍率改造

目标：让 CPU2 产生和解释的内部密度 raw 统一变为 `x100`。

建议修改：

1. 将 `DENSITY_TO_RAW(d)` 从 `d * 10.0f` 改为 `d * 100.0f`。
2. 将 `RAW_TO_DENSITY(raw)` 从 `/10.0f` 改为 `/100.0f`。
3. `DENSITY_TO_RAW()` 建议加入四舍五入：

```c
#define DENSITY_TO_RAW(d) ((uint32_t)(((d) * 100.0f) + 0.5f))
```

需要评估的边界：

- 负密度理论上无效，现有 `uint32_t` 不支持负值，保持现状。
- 如果传感器异常返回小于等于 0，继续由现有无效值逻辑处理。
- 常见密度 `0~2000 kg/m3` 改为 `0~200000`，32 位安全。
- 外部 16 位寄存器若仍输出单寄存器，需要钳位或降倍率。

### 阶段 2：CPU2 密度修正和跟随参数改造

目标：避免宏改造后参数解释错位。

涉及参数：

| 参数 | 当前 raw | 当前工程值 | 改造后 raw | 改造后工程值 |
| --- | --- | --- | --- | --- |
| `oilLevelDensity` | `8321` | `832.1` | `83210` | `832.10` |
| `oilLevelThreshold` | `15` | `1.5` | `150` | `1.50` |
| `oilLevelHysteresisThreshold` | `20` | `2.0` | `200` | `2.00` |
| `densityCorrection` | `10000` | `0.0` | `100000` 或保留基准另定 | `0.00` |

已确认策略：

1. `oilLevelDensity`、`oilLevelThreshold`、`oilLevelHysteresisThreshold` 跟随 `RAW_TO_DENSITY()` 统一升级为 `x100`。
2. `densityCorrection` 不建议继续使用基准 `10000`，建议明确为 `DENSITY_CORRECTION_BASE_RAW`：
   - 若改为 `x100`，基准建议为 `100000`，修正量为 `(densityCorrection - 100000) / 100.0f`。
   - 若为了减少参数兼容风险保留旧基准 `10000`，则 `densityCorrection` 仍只能 1 位，不满足“密度修正两位”的目标。
3. CPU3 参数菜单中对应 `point` 改为 `2`，输入输出 raw 与 CPU2 保持一致。

参数迁移建议：

- CPU2 保持 `DEVICE_PARAM_VERSION = 3`，使用旧 FRAM 中 `protocolVersion < 13` 作为一次性迁移标记。
- 在 CPU2 参数加载后增加旧协议版本迁移逻辑：
  - `oilLevelDensity != 0` 时乘以 `10`。
  - `oilLevelThreshold != 0` 时乘以 `10`。
  - `oilLevelHysteresisThreshold != 0` 时乘以 `10`。
  - `densityCorrection` 从旧 `10000 + 修正x10` 转为新 `100000 + 修正x100`，公式：

```text
new_densityCorrection = 100000 + ((old_densityCorrection - 10000) * 10)
```

本次不采用“恢复出厂参数后重新下发”的默认策略。实现时应优先迁移旧参数；只有迁移逻辑无法可靠覆盖时，才重新提交风险说明。

### 阶段 3：CPU2/CPU3 共享协议升级

目标：明确内部 Modbus 密度字段从 `x10` 改为 `x100`。

必须处理：

1. CPU2 和 CPU3 两端 `DEVICE_PROTOCOL_VERSION` 同步加 1。
2. 更新 `docs/01_协议与寄存器/CPU2_CPU3协议变更记录.md`。
3. 更新相关寄存器说明文档中的单位倍率。
4. CPU3 协议兼容检查保持严格相等。

建议协议记录内容：

| 项 | 内容 |
| --- | --- |
| 协议变化 | 密度类输入寄存器、分布点密度和密度相关参数倍率从 `kg/m3 x10` 调整为 `kg/m3 x100` |
| 地址变化 | 不变 |
| 长度变化 | 不变，仍为 32 位字段 |
| 兼容影响 | 旧 CPU3 读取新 CPU2 会把密度显示放大 10 倍；新 CPU3 读取旧 CPU2 会把密度显示缩小 10 倍，因此必须版本成对升级 |
| 旧版本处理 | 依赖 `DEVICE_PROTOCOL_VERSION` 严格相等拦截不兼容组合 |

### 阶段 4：CPU3 显示和菜单改造

目标：让 CPU3 按 2 位小数展示内部 `x100` 密度。

状态页：

1. `Display_AddCurrentPageValueStatusSlots()` 中密度槽位小数位从 `1U` 改为 `2U`。
2. `oled_equipment()` 中密度 `OledValueDisplay()` 小数位从 `1` 改为 `2`。
3. 检查 `Display_GetStatusSlotShift()` 的宽度计算是否能容纳更长的密度字符串，例如 `999.99kg/m3`。
4. `OledValueDisplay()` 会裁剪末尾 0，已确认保留裁剪策略，即 `832.10` 显示为 `832.1`，`832.12` 显示为 `832.12`。

参数菜单：

1. `液位跟随密度` 小数位从 `1` 改为 `2`。
2. `磁通量D`/密度修正小数位从 `1` 改为 `2`。
3. `密度手输值` 小数位从 `1` 改为 `2`。
4. 更新参数范围：
   - `密度手输值` 当前最大 `20000` 如果表示 `2000.0`，改为 `200000` 才能表示 `2000.00`。
   - `densityCorrection` 范围需要随基准和倍率重新定义。
5. 检查参数输入位数 `bits` 是否足够容纳新 raw。若 `83210` 需要 5 位，`200000` 需要 6 位。

### 阶段 5：外部协议适配

目标：内部升级后，不破坏 DSM、Wartsila、SI 等外部协议既有倍率；LTD 自有协议作为例外跟随两位精度。

#### 5.1 SI

当前函数：`si_density_raw_to_si_u16()`

当前逻辑：

```c
return (uint16_t)(raw_density * 10U);
```

内部升级为 `x100` 后建议：

```text
if raw_density == UNVALID_DENSITY -> 0
if raw_density > 0xFFFF -> 0xFFFF
else -> raw_density
```

说明：SI 文档当前口径为协议侧 `kg/m3 x100`，内部改成 `x100` 后不再需要额外乘 10；对 PLC 看到的寄存器值保持不变。

#### 5.2 Wartsila

当前寄存器说明为 `scale = 10`，数据结构字段名包含 `density_x10`，当前直接写内部 raw。

内部升级后建议新增转换函数：

```text
wxl_density_x10 = (density_raw_x100 + 5) / 10
```

已确认 Wartsila 继续保持 `x10`。需要处理：

- 单点密度 `density_kgm3_x10`。
- 分布点 `dens_points[i].density_x10`。
- 不改变 Wartsila 对外倍率、地址和主站解析口径。

#### 5.3 DSM

DSM 当前多处直接写内部密度 raw 到输入寄存器。

已确认 DSM 按兼容策略处理：

- DSM 对外仍输出 `x10`。
- CPU3 内部写 DSM 寄存器前执行 `(raw_x100 + 5) / 10`。
- 不改变 DSM 对外倍率、地址和主站解析口径。

#### 5.4 LTD 自有协议

LTD 自有协议是本次“不影响外部协议”原则的例外。

实施要求：

1. 搜索并确认 LTD 自有协议密度寄存器、命令或数据帧入口。
2. 将 LTD 自有协议中的密度字段同步为 `kg/m3 x100`。
3. 更新 LTD 自有协议映射文档和联调说明。
4. 如果 LTD 自有协议有独立主站或工具，需同步更新解析口径和测试用例。

#### 5.5 HART

HART 当前通过 `RAW_TO_DENSITY()` 得到 float 工程值。内部宏升级后，工程值应保持正确。

需要测试：

- 平均密度有效时 HART 返回值是否仍为正确工程密度。
- 无效值 `UNVALID_DENSITY` 是否仍按原逻辑处理。

### 阶段 6：文档同步

实现时需同步：

1. `docs/01_协议与寄存器/CPU2_CPU3协议变更记录.md`。
2. CPU2/CPU3 共享寄存器说明。
3. `docs/01_协议与寄存器/系统参数出厂默认值.md`。
4. CPU3 状态页显示说明。
5. SI 映射表和联调检查表。
6. Wartsila 寄存器映射说明，明确对外仍为 `x10`。
7. DSM 协议说明或兼容说明，明确对外倍率不变。
8. LTD 自有协议映射说明，明确跟随 `x100`。
9. `CHANGELOG.md`。
10. 版本改动与测试方案。
11. 如果流程图不受影响，版本资料中说明“不改变测量状态机和流程图，仅改变密度数值倍率、显示和协议解释口径”；如果实现过程中新增迁移流程或协议分支，则重新评估流程图是否需要更新。

## 8. 版本和兼容性影响

### 8.1 `DEVICE_PROTOCOL_VERSION`

必须升级。

理由：

- CPU2/CPU3 共享寄存器地址不变，但密度字段单位倍率从 `x10` 改为 `x100`。
- 参数字段 `oilLevelDensity`、密度阈值和密度修正的语义也可能变化。
- 旧 CPU2/CPU3 混搭会导致显示、协议输出或控制判断错误。

### 8.2 CPU2 固件版本

需要升级。

影响：

- 测量结果 raw 生成口径变化。
- 密度跟随、密度修正参数解释变化。
- 可能涉及参数存储迁移。
- 内部 Modbus 发布语义变化。

建议版本级别：

- 如果做兼容迁移且地址/流程不变：至少 `minor`。
- 如果选择清参数或外部协议不兼容：按项目版本规则评估为 `major` 或明确不兼容升级。

### 8.3 CPU3 固件版本

需要升级。

影响：

- 状态页密度显示两位。
- 参数菜单密度输入两位。
- CPU2 内部协议版本匹配。
- 外部协议边界转换变化。

建议版本级别：

- 新增协议语义支持和显示口径变化，至少 `minor`。

### 8.4 CPU2 参数存储版本

密度参数 raw 语义变化后，CPU2 已确认不升级 `DEVICE_PARAM_VERSION`，改用 `protocolVersion < 13` 作为一次性迁移标记。

两种策略：

| 策略 | 优点 | 缺点 |
| --- | --- | --- |
| 协议版本标记迁移旧参数 | 保留现场参数，不改变 CPU2 参数存储版本 | 需要确保 `protocolVersion` 先迁移后写回 13，避免重复迁移 |
| 升级 `DEVICE_PARAM_VERSION` 后迁移 | 迁移标记清晰 | 会改变 CPU2 参数存储版本，发布资料和现场解释成本更高 |

已确认采用协议版本标记迁移旧参数，并在升级日志中说明迁移字段和公式。

## 9. 验证计划

### 9.1 静态检查

1. 搜索 `DENSITY_TO_RAW`、`RAW_TO_DENSITY`，确认所有调用点适配新倍率。
2. 搜索密度显示 `OledValueDisplay`，确认状态页密度小数位为 2。
3. 搜索 `density_x10`、`kgm3_x10`，确认外部协议边界做了转换。
4. 搜索 SI协议密度转换，确认内部 `x100` 不再额外乘 10。
5. 搜索 DSM 密度输出，确认对外仍按现有倍率输出。
6. 搜索 Wartsila 密度输出，确认对外仍按 `x10` 输出。
7. 搜索 LTD 自有协议密度输出，确认按 `x100` 输出。
8. 检查 `DEVICE_PROTOCOL_VERSION` CPU2/CPU3 一致。
9. 检查 `DEVICE_PARAM_VERSION` 和迁移逻辑一致。

### 9.2 构建验证

```powershell
cmake --build build\LTD_MAIN_CPU2
cmake --build build\LTD_DISPLAY_CPU3
```

如果改动 SI协议适配，补充：

```powershell
py tools\check_si_modbus_frames.py
py tools\check_si_protocol_contract.py
```

如存在版本检查脚本并进入提交阶段：

```powershell
py tools\check_version_bumped.py
```

### 9.3 功能验证

| 场景 | 输入/准备 | 期望 |
| --- | --- | --- |
| 单点测量 | 传感器返回 `832.12 kg/m3` | CPU2 raw 为 `83212`，CPU3 显示 `832.12 kg/m3` |
| 单点监测 | 连续读数变化 `832.10 -> 832.12` | CPU3 可体现第二位变化 |
| 分布测量平均 | 多点 raw 为 `83212`、`83213`、`83214` | 平均 raw 和显示保留两位 |
| 瓦锡兰分布点 | 内部 raw `83212` | Wartsila 若保持 `x10`，输出 `8321` |
| SI 当前密度 | 内部 raw `83212` | SI 输出 `83212`，不再输出 `832120` |
| DSM 密度输出 | 内部 raw `83212` | 对外倍率保持现有口径 |
| LTD 自有协议密度输出 | 内部 raw `83212` | 按两位精度 `x100` 输出 |
| 液位跟随密度 | 菜单输入 `832.12` | CPU2 参数保存 `83212`，跟随目标为 `832.12` |
| 密度修正 | 菜单输入 `+0.12` | 实际密度加 `0.12 kg/m3` |
| 旧 FRAM 升级 | 旧参数 `oilLevelDensity=8321` | 迁移后为 `83210`，工程值保持 `832.10` |

### 9.4 回归验证

1. 液位密度跟随不会因阈值倍率变化导致动作过敏或不动作。
2. 密度修正不会被放大或缩小 10 倍。
3. CPU3 参数菜单输入、确认、回读和保存一致。
4. CPU3 状态页分页和右移计算不因多一位小数导致重叠。
5. DSM/Wartsila/SI 主站读取密度值符合各自既有协议文档，倍率不变。
6. LTD 自有协议读取密度值符合新的两位精度口径。
7. 协议版本不匹配时 CPU3 能阻止旧 CPU2/新 CPU3 或新 CPU2/旧 CPU3 混用。

## 10. 风险和规避

| 风险 | 影响 | 规避 |
| --- | --- | --- |
| 只改显示小数位 | 密度显示缩小 10 倍 | 必须先统一 raw 倍率 |
| 外部协议继续透传内部 raw | Wartsila/DSM 可能读到放大 10 倍的值 | DSM/Wartsila/SI 在协议边界显式转换或取消多余转换 |
| SI 继续 `*10` | SI 输出放大 10 倍 | 内部升级后 SI 只钳位不乘 10 |
| 旧 FRAM 参数未迁移 | 密度目标、阈值、修正量解释错误 | 升级参数版本并迁移或明确清参数 |
| 密度修正基准未同步 | 修正量放大/缩小 10 倍 | 定义新基准常量并统一公式 |
| CPU2/CPU3 版本混搭 | 显示和协议输出错误 | 升级 `DEVICE_PROTOCOL_VERSION` 并严格匹配 |
| OLED 显示宽度不足 | 数值和单位重叠 | 验证最大值字符串和分页布局 |
| 文档未同步 | 上位机或现场按旧倍率解析 | 同步协议记录、映射表和测试方案；明确哪些协议保持原倍率，哪些协议升级为 `x100` |

## 11. 回退方案

如果实现后现场确认两位精度影响外部系统兼容，可按以下顺序回退：

1. 保持 CPU2/CPU3 内部 `x100` 不变，仅把外部 DSM/Wartsila 输出降回 `x10`。
2. 如果 CPU3 状态页显示两位影响布局，可临时显示 `x100` 数据但限制为 1 位，内部精度保留。
3. 如果参数两位输入影响现场操作，可保留测量结果 `x100`，但密度跟随参数继续按 `x10`，同时在文档中明确“测量结果两位，参数一位”。该方案不推荐作为最终方案，因为口径不统一。
4. 若必须完全回退，则恢复 `DENSITY_TO_RAW/RAW_TO_DENSITY` 到 `x10`，回退协议版本和参数迁移逻辑，并确保新旧 FRAM 不被错误解释。

## 12. 待确认事项

已确认事项见第 3 节。后续实现前只剩以下工程细节需要在源码中确认：

1. LTD 自有协议的具体源码入口、寄存器/帧字段和文档位置。
2. DSM 当前“现有对外倍率”的精确倍率，以源码和协议文档为准实现边界转换。
3. CPU3 密度菜单最大值、最小值和输入位数是否需要随 `x100` 扩展。
4. 旧 FRAM 迁移函数放置位置和迁移版本号。

## 13. 推荐结论

已确认采用以下最终口径：

1. CPU2/CPU3 内部密度 raw 统一改为 `kg/m3 x100`。
2. CPU3 状态页和密度相关参数菜单改为 2 位小数。
3. DSM、Wartsila、SI 等既有外部协议对外倍率保持不变。
4. SI 保持协议侧 `x100`，内部升级后取消额外乘 10。
5. Wartsila 保持协议 `x10`，输出前从内部 `x100` 转为 `x10`。
6. DSM 保持既有外部协议口径，输出前按现有倍率转换。
7. LTD 自有协议作为例外，跟随内部两位精度口径升级为 `x100`。
8. 升级 CPU2/CPU3 `DEVICE_PROTOCOL_VERSION`。
9. 保持 CPU2 `DEVICE_PARAM_VERSION = 3`，使用 `protocolVersion < 13` 迁移旧密度参数，避免现场参数丢失和重复迁移。
10. 实现和提交阶段同步版本、CHANGELOG、协议变更记录和版本改动与测试方案。

该方案改动面较大，但边界清晰：内部和 LTD 自有协议统一两位，其它外部协议按既有契约转换。这样可以满足显示和内部精度目标，同时降低对现有主站和现场参数的冲击。

## 14. 落地记录

落地日期：2026-06-26

### 14.1 已实现内容

1. CPU2/CPU3 `DEVICE_PROTOCOL_VERSION` 从 `12` 升级到 `13`。
2. CPU2 内部密度 raw 从 `kg/m3 x10` 改为 `kg/m3 x100`，`DENSITY_TO_RAW()` 和 `RAW_TO_DENSITY()` 同步更新。
3. CPU2 `DEVICE_PARAM_VERSION` 保持 `3`，通过旧 FRAM 中 `protocolVersion < 13` 判断并迁移以下字段：
   - `oilLevelDensity`
   - `oilLevelThreshold`
   - `oilLevelHysteresisThreshold`
   - `densityCorrection`
4. CPU2 `densityCorrection` 基准从 `10000` 改为 `100000`，修正量公式改为 `(raw - 100000) / 100.0f`。
5. CPU2 出厂默认值中密度阈值改为 `150` / `200`，密度修正默认值改为 `100000`。
6. CPU2 频率找液兼容路径会把迁移后的阈值按 x10 折算回 Hz，避免旧阈值被放大 10 倍。
7. CPU3 状态页密度显示改为 2 位小数，保留尾零裁剪。
8. CPU3 参数菜单中密度相关项改为两位口径：
   - `液位找液阈值`
   - `液位滞后阈值`
   - `液位跟随密度`
   - `磁通量D`
   - `密度手输值`
9. CPU3 本机 FRAM 参数版本从 `0x0004` 升级到 `0x0005`，读取 `0x0003` / `0x0004` 时迁移 `screen_input_d`。
10. DSM 外部协议密度输出保持旧 x10 口径，`densityCorrection` 对外仍保持旧基准 `10000`。
11. Wartsila 外部协议密度输出保持 `scale = 10` / x10。
12. SI协议密度保持 `0.01 kg/m3` 口径，内部升级后取消额外乘 10。
13. 协议变更记录和出厂默认值文档已同步协议 13 和新密度倍率。

### 14.2 发布阶段仍需处理

1. 如进入正式版本发布，需要同步 CPU2/CPU3 固件版本号。
2. 如进入正式版本发布，需要补充 `CHANGELOG.md` 和 `docs/00_构建与版本/版本改动与测试/` 下的版本改动与测试方案。
3. 需要硬件或仿真联调确认 OLED 显示宽度、旧 FRAM 迁移和 DSM/Wartsila/SI 主站倍率。
