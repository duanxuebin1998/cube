...
适用版本：当前源码共享协议版本 `14`；读取部件参数页 RSSI 显示从协议版本 `9` 起支持，密度两位小数显示从协议版本 `13` 起支持，SI Profile 来源隔离和点阵输入寄存器从协议版本 `14` 起支持。CPU3 与 CPU2 通信超时故障为 CPU3 本机显示和故障状态修复，不改变共享协议版本
...
| CPU2通信超时 | `STATE_ERROR` 且 `error_code == CPU2_COMM_TIMEOUT` | `CPU2_CommRecordFailure()`、`CPU2_CommShouldShowStartup()`、`CPU2_CommIsAvailable()`、`CPU2_CommCanSendCommand()` | 连续请求失败计数从 0 开始；1000 ms 超时、非法响应、UART 错误和 TX DMA 启动失败均按 1 次累计。冷启动第 1～9 次显示“通讯尝试中...”，第 10 次置 `STATE_ERROR` 和 `0x0013000A`；只有完整包含设备状态和错误码的 0x04 响应才能解除故障锁存。通讯尝试页要等状态和当前连接协议快照都建立后才退出，避免协议字段未读回时显示伪 `ProtoErr` 或掉线前兼容结果。故障会清参数及协议快照，状态恢复后必须重新收到覆盖协议字段的合法 0x03。参数刷新期间普通写关闭，但状态/协议快照有效、协议为 14 且无故障时，屏幕 `CMD_CANCEL_MEASUREMENT` 仍可达；SI `00009 Stop` 不使用该绕过 |
