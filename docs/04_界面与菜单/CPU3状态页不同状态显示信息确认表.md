...
适用版本：当前开发及最新正式固件组合为共享协议 `23`、CPU2 `V1.28.0.0` / CPU3 `V1.26.0.0`；读取部件参数页RSSI显示从协议9起支持，密度两位小数显示从协议13起支持，SI Profile来源隔离和点阵输入寄存器从协议14起支持，SI生命周期字段从协议18起支持。协议19拆分多义故障，协议20新增AO配置和运行态，协议21增加固定点结果代际和已确认快照，协议22只重排32项共享故障编号，协议23收敛AO故障动作和非跟随输出语义。协议21及更早固件必须按对应CPU2历史页解释故障编号；协议22及更早版本不能按协议23解释AO故障动作、原错误等级槽位和非跟随电流字段，协议20及更早版本缺少协议21固定点快照契约。
...
| CPU2通信超时 | `STATE_ERROR` 且 `error_code == CPU2_COMM_TIMEOUT` | `CPU2_CommRecordFailure()`、`CPU2_CommShouldShowStartup()`、`CPU2_CommIsAvailable()`、`CPU2_CommCanSendCommand()` | 连续请求失败计数从0开始；1000 ms超时、非法响应、UART错误和TX DMA启动失败均按1次累计。冷启动第1～9次显示“通讯尝试中...”，第10次置 `STATE_ERROR` 和 `0x00140007`，屏幕显示 `20-7`；只有完整包含设备状态和错误码的0x04响应才能解除故障锁存。通讯尝试页要等状态和当前连接协议快照都建立后才退出，避免协议字段未读回时显示伪 `ProtoErr` 或掉线前兼容结果。故障会清参数及协议快照，状态恢复后必须重新收到覆盖协议字段的合法0x03。参数刷新期间普通写关闭，但状态/协议快照有效、协议匹配且无故障时，屏幕 `CMD_CANCEL_MEASUREMENT` 仍可达；SI `00009 Stop`不使用该绕过 |
...
| CPU2通信超时故障 | 状态快照、完整参数快照、当前连接协议快照、连续请求失败和故障恢复锁存已拆分；连续第10次未获得合法响应才进入 `STATE_ERROR`，错误码 `0x00140007`，屏幕显示 `20-7`。上电或恢复先独立读取`0x0010~0x0011`协议字段，协议兼容后才读取状态、完整参数和扩展运行态；协议不匹配时只保留版本心跳并显示“协议版本不匹配”。故障会清参数和协议快照，依赖CPU2的普通写入口必须等待三类快照完整且共享协议版本相等。任何已发起的非命令FC16失败，或FC16已返回合法ACK但紧随的定向FC03补读失败，都会立即关闭参数快照并强制补读四组。主循环可调度时按100 ms门限检查轮询，外部流量不能永久饿死CPU2 | 冷启动分别复测第1、9、10次失败；覆盖超时、非法帧、UART/TX失败；交叉烧写协议18/19/20 CPU2，确认只读取稳定协议地址、不访问新扩展区、不累计通信超时并显示协议不匹配；状态0x04恢复后应继续显示通讯尝试页且屏幕取消被拒绝，最后返回覆盖协议字段且值为21的合法0x03，确认完整参数补读前普通写仍关闭；对菜单/DSM/SI/Wartsila参数写注入ACK丢失，并对菜单定向补读注入超时或CRC错误，确认同轮普通命令被阻止且强制补读不依赖更新标志；Wartsila补读前四参数读返回 `0x06`，补读后返回CPU2实际值；持续外部流量下记录含1000 ms同步阻塞在内的最坏轮询间隔 |
