...
3. 使用 `SET_ERROR(ret)` 记录分布测量本身的错误码。
4. 将本次分布测结果写入 `g_measurement.density_distribution`。
5. 延时 1 秒，期间允许命令切换打断。
6. 打印分布测量结果。
7. 设置设备状态为 `STATE_WARTSILA_DENSITY_OVER`。
8. 延时 8 秒，给 CPU3 读取分布测量结果，期间允许命令切换打断。
9. 判断 `g_deviceParams.wartsila_bottom_detect_interval`，确定本次是否需要测后探底。
10. 如果本次不需要探底，切回 `CMD_MONITOR_SINGLE`。
11. 如果本次需要探底，先移动到固定点监测位置。
12. 固定点移动完成后，调用 `SearchBottom()` 执行罐底测量。
13. 罐底测量完成且偏差合格时，执行编码器修正。
14. 流程结束后切回 `CMD_MONITOR_SINGLE`，继续单点监测。
...
2. 如果新增保持寄存器或改变共享协议，需要同步提升 `DEVICE_PROTOCOL_VERSION`，并更新 `docs/CPU2_CPU3协议变更记录.md`。
