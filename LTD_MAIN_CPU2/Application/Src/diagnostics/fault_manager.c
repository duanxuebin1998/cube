...
/** 故障发生位置和错误码的私有快照。 */
typedef struct {
    const char *file;   /* 触发故障的短文件名，指向静态字符串。 */
    uint32_t line;      /* 触发故障的源码行号。 */
    const char *func;   /* 触发故障的函数名，指向静态字符串。 */
    uint32_t error_code; /* 需要发布或返回的整机错误码。 */
} ErrorInfo;

static ErrorInfo s_error_context; /* 仅由故障管理入口更新的当前错误现场。 */
 ...
 *
 * @param error_info 待上报错误信息对象；函数读取其中的错误码，传入 NULL 时直接返回。
 * @note 传入 NULL 时直接返回；统一报错出口会过滤 NO_ERROR 和 STATE_SWITCH，并对短时间内的重复错误码去重，本函数自身不执行停机。
 */
static void FaultManager_PrintError(const ErrorInfo *error_info)
{
    if (error_info == NULL) return; /* 空指针保护 */

    FaultManager_ReportErrorExit(error_info->error_code);
}
...
void FaultManager_ReportErrorExit(uint32_t error_code)
{
    ...
    snprintf(detail, sizeof(detail),
             "文件：%s,行号：%lu,函数：%s",
             (s_error_context.file != NULL) ? s_error_context.file : "未知",
             (unsigned long)s_error_context.line,
             (s_error_context.func != NULL) ? s_error_context.func : "未知");
...
uint32_t FaultManager_HandleCheckError(uint32_t error_code,
                                       ...
                                       const char *func)
{
    s_error_context.file = file;
    s_error_context.line = line;
    s_error_context.func = func;
    s_error_context.error_code = error_code;

    FaultManager_ReportErrorExit(s_error_context.error_code);
    HandleError();
    return s_error_context.error_code;
}
                                        ...
                                        const char *func)
{
    ...
    FaultManager_ReportGlobalErrorExit(s_error_context.error_code, file, line, func);
    HandleError();
    return s_error_context.error_code;
}
                                        ...
                                        const char *func)
{
    ...
    /* 保持历史顺序：先执行安全停机，再输出最终可读错误日志。 */
    HandleError();
    FaultManager_PrintError(&s_error_context);
    return s_error_context.error_code;
}
...
{
    s_error_context.error_code = STATE_SWITCH;
    HandleError();
    return s_error_context.error_code;
}
