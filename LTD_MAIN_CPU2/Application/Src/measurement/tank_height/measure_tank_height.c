 ...
 * @return 返回探底流程允许使用的最大尺带长度，单位 0.1 mm的有效长度，单位字节；0 表示没有可供消费的数据。
 */
static uint32_t Bottom_GetMaxCableLength01mm(void)
{
 ...
 * @return 返回根据触底尺带长度和标定偏移计算罐高的有效长度，单位字节；0 表示没有可供消费的数据。
 */
static uint32_t BuildTankHeightFromCableLength(int32_t cable_length_01mm)
{
 ...
 * @return 返回根据罐高和标定偏移反算触底尺带长度的有效长度，单位字节；0 表示没有可供消费的数据。
 */
...
uint32_t SearchBottom(void)
{
    ...
    printf("罐底测量\t粗找罐底完成：实高：%ld mm", TankHeight_GetBottomCableLength01mm()); MotorCtrl_PrintPositionRefs(); printf("\r\n");
...
static int SearchBottomRough() {
    ...
    {
        /* 记录首次检测到的罐底位置 */
        TankHeight_SetBottomCableLength01mm(g_measurement.debug_data.cable_length);
        return NO_ERROR;
    }
    else {
        ret = MotorCtrl_MoveBlockingNoDetect(100.0, MOTOR_DIRECTION_UP, MotorCtrl_GetDefaultSpeedX100());
...
static int SearchBottomPrecise() {
    ...
    while (1) {
        ...
        if (TankHeight_GetBottomCableLength01mm()-g_measurement.debug_data.cable_length < 100) {
            speed_x100 = 10;
        }
        else if (TankHeight_GetBottomCableLength01mm()-g_measurement.debug_data.cable_length  < 300) {
            speed_x100 = 40;
        }
        else if (TankHeight_GetBottomCableLength01mm()-g_measurement.debug_data.cable_length  < 3000)  {
            speed_x100 = 100;
        }
        ...
        if (TankHeight_GetBottomCableLength01mm()-g_measurement.debug_data.cable_length < -1000)  {
