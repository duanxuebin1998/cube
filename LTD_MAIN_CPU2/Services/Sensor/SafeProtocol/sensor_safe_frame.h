...
/*
 * 函数用途：从连续字节流头部识别控制帧或固定快报帧的期望长度。
 * 调用场景：UART6 接收状态机找到 A5 5A 后决定还需接收的字节数。
 * 关键约束：只读取长度字段，不代表帧已通过 CRC 或语义校验。
 */
...
/*
 * 函数用途：逐字段编码一个 V1 控制帧并计算 CRC32C。
 * 调用场景：CPU2 发送 HELLO、测量、维护和会话控制请求。
 * 关键约束：不对 C 结构体内存求 CRC，输出长度不得超过 128 字节。
 */
...
/*
 * 函数用途：校验并解码一个完整控制帧。
 * 调用场景：UART6 收满 frame_len 后，在会话层使用响应前调用。
 * 关键约束：返回成功前已校验 SOF、长度、版本、CRC、消息类型和保留标志位。
 */
...
/*
 * 函数用途：校验并解码固定 44 字节快报帧。
 * 调用场景：周期上报接收路径进入会话连续性检查之前。
 * 关键约束：只接受 PERIODIC_UPLINK 和可用 4 bit 表示的测量模式 0..5。
 */
...
/*
 * 函数用途：解码所有 RSP/ERR 共有的 20 字节响应前缀。
 * 调用场景：控制帧已通过帧层和会话层校验后解析业务负载。
 * 关键约束：该函数不替代状态位、诊断位和数据年龄的安全判断。
 */
