/*
 * 模块职责：声明DSM文本协议的数据结构、版本画像和模式/测量/维护接口。
 * 调用边界：业务层优先通过SensorService调用，本头文件保留协议驱动和兼容接口。
 * 返回约束：函数继续使用现有全局错误码；有效的0值与通信超时、格式错误必须分别表达。
 */
...
typedef enum
{
	DSM_VERSION_PROFILE_UNREAD = 0, /* 本次会话尚未完成版本画像读取。 */
	DSM_VERSION_PROFILE_CV07_AMBIGUOUS, /* CV返回07，仍需Cv细分真实代际。 */
	DSM_VERSION_PROFILE_V3, /* DSM V3版本画像。 */
	DSM_VERSION_PROFILE_V4, /* DSM V4版本画像。 */
	DSM_VERSION_PROFILE_V5_0_AMBIGUOUS, /* DSM V5.0兼容响应，功能能力仍需命令验证。 */
	DSM_VERSION_PROFILE_V5, /* DSM V5版本画像。 */
	DSM_VERSION_PROFILE_V6, /* DSM V6版本画像。 */
	DSM_VERSION_PROFILE_UNKNOWN /* 已收到版本文本但无法归入已知画像。 */
} DsmVersionProfile;
