/*
 * 模块职责：实现DSM一代文本命令的固定长度收发、BCC校验、版本画像和测量值解析。
 * 传输边界：每次事务显式申请UART6-DSM所有权，并在无线透明链路就绪后启动DMA收发。
 * 兼容约束：命令响应长度由契约表确定；业务解析失败只在有界重试内重发，不回退为按换行截帧。
 */
...
typedef enum {
    DSM_FRAME_ACK = 0, /* 仅返回确认字符的模式或功能命令。 */
    DSM_FRAME_COMBINED_VERSION, /* CV组合版本响应。 */
    DSM_FRAME_CPU1_VERSION, /* Cv传感器CPU1版本响应。 */
    DSM_FRAME_SENSOR_NUMBER, /* CN传感器编号响应。 */
    DSM_FRAME_SUPPLY_VOLTAGE, /* CK供电电压响应。 */
    DSM_FRAME_LEVEL_FREQUENCY, /* Cb液位频率响应。 */
    DSM_FRAME_DENSITY, /* Cd频率、密度和温度响应。 */
    DSM_FRAME_DENSITY_ANALYSIS, /* CM密度分析扩展响应。 */
    DSM_FRAME_GYRO_ANGLE, /* Ch双轴倾角响应。 */
    DSM_FRAME_WATER_CAPACITANCE /* Cl水位电容响应。 */
} DsmFrameType;

/* 一条DSM命令对应的固定响应长度和解析类型契约。 */
typedef struct {
    const char *command; /* 不含BCC和换行的两字符DSM命令文本。 */
    uint16_t normal_len; /* 正常响应完整长度，单位字节。 */
    uint16_t error_len; /* 统一错误响应完整长度；0表示该命令没有错误载荷格式。 */
    DsmFrameType frame_type; /* 响应通过线校验后使用的业务解析类型。 */
    const char *operation_name; /* 错误重试和原包日志使用的中文操作名称。 */
} DsmCommandFrameSpec;
...
static int UART6_SendWithPolicy(const char *cmd,
    ...
    for (uint32_t i = 0U; i < max_attempts; i++) {
        ...
        if (SensorUart6Owner_Acquire(SENSOR_UART6_OWNER_DSM) == 0U) {
            return SENSOR_MODE_NOT_READY;
        }
        ret = UART6_SendCommand(frame_spec, response, maxLen, &recvLen, timeout);
        (void)SensorUart6Owner_Release(SENSOR_UART6_OWNER_DSM);
        if (trace_probe != 0U) {
