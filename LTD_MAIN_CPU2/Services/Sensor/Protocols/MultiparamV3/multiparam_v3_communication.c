/*
 * 模块职责：实现多参数V3固定8字节请求/应答、寄存器读取和密度/液位模式控制。
 * 传输边界：事务独占UART6-V3所有权；地址、功能码、参数码和累加校验均通过后才解析数据。
 * 兼容约束：V3与DSM、DM4-V4互为独立驱动，不共享帧格式或协议状态。
 */
...
static const char *s_multiparam_v3_last_stage = "未开始"; /* 最近V3事务失败或结束阶段的静态文本。 */
 ...
 */
static int MULTIPARAM_V3_TransceiveOwned(const uint8_t tx[8], uint8_t rx[8]) {
...
static int MULTIPARAM_V3_Transceive(const uint8_t tx[8], uint8_t rx[8])
{
    ...
    if (SensorUart6Owner_Acquire(SENSOR_UART6_OWNER_MULTIPARAM_V3) == 0U) {
        return SENSOR_MODE_NOT_READY;
    }
    result = MULTIPARAM_V3_TransceiveOwned(tx, rx);
    (void)SensorUart6Owner_Release(SENSOR_UART6_OWNER_MULTIPARAM_V3);
    return result;
}
