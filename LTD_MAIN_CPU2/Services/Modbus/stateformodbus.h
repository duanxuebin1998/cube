...
/* AO 运行状态块中“DAC 回读电流，单位 0.001 mA”字段的起始地址；CPU3 读取时应结合更新计数和有效标志解释该值。 */
 ...
 * @param start 本次连续访问区间的起始寄存器或位地址。
 * @param count 参与本次处理的数据项数量。
 * @return true 表示寄存器数量非零、完整区间未越界且每个地址均属于当前可写保持寄存器；false 表示数量为零、地址加法越界、区间超出表尾，或包含只读/保留地址。
 */
 ...
 * @param start 本次连续访问区间的起始寄存器或位地址。
 * @param count 参与本次处理的数据项数量。
 * @return true 表示非空 FC10 区间完整落在七个带参命令的前置参数连续区域内；false 表示区间为空、起始过早、结束越界或混入其他字段。
 */
 ...
 * @param start 本次连续访问区间的起始寄存器或位地址。
 * @param count 参与本次处理的数据项数量。
 * @return true 表示写区间命中至少一个需要持久化的设备参数；false 表示区间无效，或只覆盖命令、AO 仿真开关及四路清锁存报警瞬时写槽。
 */
...
/**
 * @brief 这里只接受已经具备CPU2业务处理闭环的命令。
 *
 * @param command 准备由 LTD Modbus 线圈触发的 32 位 CommandType 编码；仅已形成 CPU2 业务闭环的命令返回已实现。
 * @return true 表示命令号已具备 CPU2 入口、执行和完成/错误反馈闭环；false 表示命令未知、预留或尚未实现，必须拒绝写入。
 */
