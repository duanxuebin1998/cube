...
### 任务
- [ ] 新增 `LTD_MAIN_CPU2/docs/02_通信与解耦/解耦基线记录.md`：记录命令行为、状态流转、关键帧样例。
- [ ] 新增 `LTD_MAIN_CPU2/docs/02_通信与解耦/CubeMX编辑边界.md`：定义“可改文件与不可改区域”。
- [ ] 约定改动规则：
...
- `BSP/Peripherals/*`（仅作为底层依赖，不让上层直接 include）
...
- [ ] `git diff -- Core Drivers BSP`（确保仅 USER CODE 允许改动）
...
1. 补齐 `解耦基线记录.md` 与 `CubeMX编辑边界.md`。
