...
def wartsila_si_page() -> dict:
    return {
        ...
        "overview_text": "这两个协议都是 CPU3 的对外适配层：Wartsila 写入会向 CPU2 下发参数，SI 读写先刷新本地快照。",
        ...
        "overview_nodes": [
            ...
            {"id": "g", "kind": "process", "x": 730, "y": 500, "w": 300, "h": 90, "label": "SI 按功能码读写线圈/寄存器并生成异常或正常响应"},
        ...
        "flows": [
            ...
            {
                "title": "SI 快照同步与功能码处理",
                "caption": "SI 在地址和 CRC 通过后才刷新 CPU3 快照，读写函数码都会生成正常或异常响应；分发层只要有响应帧就视为成功。",
                "height": 1040,
                "nodes": [
                ...
                "evidence": [
                    {"title": "快照刷新", "text": f"{code('si_modbus_slave.c:997-1012')} si_modbus_sync_from_system 汇总刷新四类数组。"},
                    {"title": "处理入口", "text": f"{code('si_modbus_slave.c:1016-1080')} 地址/CRC 通过后才同步系统状态并按功能码处理。"},
                    {"title": "分发表适配", "text": f"{code('si_modbus_slave.c:1083-1096')} 只要已生成响应帧，COM 分发层就按成功处理并发送。"},
                ],
            },
        ],
        "issues": [
            ...
            {"level": "mid", "title": "SI 写操作是否同步 CPU2 需要单独核对", "desc": "SI 写单线圈/单寄存器在模块内处理，页面证据显示主入口负责快照和响应，写后对 CPU2 的影响需要按具体 handler 继续核对。", "suggest": "把 SI 05/06 的业务动作整理成协议表，区分只改本地快照、下发 CPU2、占位回显三类。", "ref": "si_modbus_slave.c:1016-1080"},
        ...
        "sources": [
            ...
            {"title": "si_modbus_process", "desc": "SI 地址/CRC/快照/功能码处理。", "refs": ["Communication/external/si_modbus/si_modbus_slave.c:1016-1080"]},
