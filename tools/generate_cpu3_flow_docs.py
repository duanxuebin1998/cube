        ...
        "flows": [
            ...
            },
            {
                ...
                "evidence": [
                    ...
                    {"title": "协议版本", "text": f"{code('Application/system_param/system_parameter.h:29')} 当前 DEVICE_PROTOCOL_VERSION 为 14，CPU3 使用严格相等判断协议兼容。"},
                ],
            },
        ],
        "issues": [
...
def param_fram_io_page() -> dict:
    return {
        ...
        "short": "本机参数与外设恢复",
        "hero": "梳理 CPU3 本机通信/显示参数默认值、FRAM V3/V4/V5 到当前 V6（0x0006）的迁移、CRC 校验、串口重配、RTC 初始化、OLED SPI 恢复和 UART 错误恢复。",
        ...
        "overview_description": "CPU3 应用初始化时，RTC 优先启用 LSE、失败回退 LSI，并依据备份标记保留校时时间，同时从 FRAM 读取本机参数；magic、版本和 CRC 有效时加载或迁移，否则恢复默认并重新保存。参数应用后重配 COM1/2/3 进入运行；后续保存会重建 CRC 镜像，运行期串口或 OLED 错误分别触发接收或清屏恢复。",
        ...
        "flows": [
            {
                ...
                "evidence": [
                    ...
                    {"title": "保存判重", "text": f"{code(func_line(FRAM_SOURCE, 'Cpu3_Params_SaveToFRAM'))} 保存前构造 V6 目标镜像，现有有效镜像参数相同则跳过写入。"},
                ],
            },
            {
                "title": "RTC 与外设错误恢复",
        ...
        "sources": [
            ...
            {"title": "Cpu3_Params_SaveToFRAM", "desc": "CPU3 本机参数 V6 镜像构造、CRC、判重和写入。", "refs": [func_line(FRAM_SOURCE, "Cpu3_Params_SaveToFRAM"), func_line(FRAM_SOURCE, "Cpu3_Params_BuildStorage")]},
