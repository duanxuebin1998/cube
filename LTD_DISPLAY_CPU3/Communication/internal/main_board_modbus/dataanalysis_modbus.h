...
/**
 * @brief 执行Modbus 协议中的 AnalysisHoldRegister 逻辑。
 */
void AnalysisHoldRegister(void); /* 解析保持寄存器数据 */

/**
 * @brief 写入或设置Modbus 协议中的 WriteDeviceParamsToHoldingRegisters 逻辑。
 *
 * @param HoldingRegisterArray 业务参数。
 */
void WriteDeviceParamsToHoldingRegisters(uint16_t *HoldingRegisterArray); /* 写入保持寄存器 */
/**
 * @brief 读取Modbus 协议中的 ReadDeviceParamsFromHoldingRegisters 逻辑。
 *
 * @param HoldingRegisterArray 业务参数。
 */
