...
static uint8_t *dtm_operaname_short(int num, uint8_t *fallback)
{
	static const OperaShortNameMap_t short_map[] = {
		...
		{ COM_NUM_DEVICEPARAM_LEVEL_REFERENCE_REFRESH_ENABLE, (uint8_t*)"定时矫正", (uint8_t*)"TimeCorr" },
		{ COM_NUM_DEVICEPARAM_LEVEL_REFERENCE_REFRESH_INTERVAL_MIN, (uint8_t*)"矫正周期", (uint8_t*)"CorrPer" },
...
static bool operation_needs_protect_confirm(int operaNum)
{
	...
	case COM_NUM_DEVICEPARAM_LEVEL_REFERENCE_REFRESH_ENABLE:
...
static MenuGroup ParamGroupOf(int operaNum)
{
    ...
    case COM_NUM_DEVICEPARAM_LEVEL_REFERENCE_REFRESH_ENABLE:
    case COM_NUM_DEVICEPARAM_LEVEL_REFERENCE_REFRESH_INTERVAL_MIN:
