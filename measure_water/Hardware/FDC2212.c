 ...
 */
void FDC2112_Set_Reg(uint8_t Slve_Addr, uint8_t read_REG_addr, uint16_t data)
{
	uint8_t TXbuffer[10] = {0,};
	TXbuffer[0] = read_REG_addr;
	TXbuffer[1] = (uint8_t)(data>>8);
	TXbuffer[2] = (uint8_t)data;
	HAL_I2C_Master_Transmit(&hi2c2, Slve_Addr, (uint8_t*) &TXbuffer, 3, 1000);
}
 ...
 *  @return     寄存器值
 *  @since      v1.0
 *  Sample usage:       FDC2112_Read_Reg(FDC2214_Addr, FDC2214_CONFIG);
 */
uint16_t FDC2112_Read_Reg(uint8_t Slve_Addr, uint8_t read_REG_addr)
{
	uint16_t data = 0;
	...
	TXbuffer[0] = read_REG_addr; //ADDRES_DATA0_REG;
	HAL_I2C_Master_Transmit(&hi2c2, Slve_Addr, (uint8_t*)TXbuffer, 1, 1000);
	HAL_I2C_Master_Receive (&hi2c2, Slve_Addr, (uint8_t*)&RXbuffer[0], 2, 1000);
	data |= RXbuffer[0]<<8;
	data |= RXbuffer[1]<<0;
	return data;
}
...
uint8_t FDC2214_GetChannelData(FDC2214_channel_t channel, uint32_t *data)
{
    ...
    status = FDC2112_Read_Reg(FDC2214_Addr, FDC2214_STATUS);//读取状态寄存器的值
    while(timeout && !(status & bitUnreadConv))//检查是否有未读数据
    {
        status = FDC2112_Read_Reg(FDC2214_Addr, FDC2214_STATUS);
        timeout--;
    }
    if(timeout)
    {
        dataRead = (uint32_t)(FDC2112_Read_Reg(FDC2214_Addr, dateReg_H) << 16);
        dataRead |= FDC2112_Read_Reg(FDC2214_Addr, dateReg_L);
        *data = dataRead;
        return 1;
    }
    else
...
void FDC2214_SetRcount(FDC2214_channel_t channel, uint16_t rcount)
{
    ...
    {
        case FDC2214_Channel_0:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_RCOUNT_CH0, rcount);
        break;

        case FDC2214_Channel_1:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_RCOUNT_CH1, rcount);
        break;

        case FDC2214_Channel_2:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_RCOUNT_CH2, rcount);
        break;

        case FDC2214_Channel_3:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_RCOUNT_CH3, rcount);
        break;
    }
}
...
void FDC2214_SetSettleCount(FDC2214_channel_t channel, uint16_t count)
{
    ...
    {
        case FDC2214_Channel_0:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_SETTLECOUNT_CH0, count);
        break;

        case FDC2214_Channel_1:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_SETTLECOUNT_CH1, count);
        break;

        case FDC2214_Channel_2:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_SETTLECOUNT_CH2, count);
        break;

        case FDC2214_Channel_3:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_SETTLECOUNT_CH3, count);
        break;
    }
}
...
void FDC2214_SetChannelClock(FDC2214_channel_t channel, uint8_t frequency_select, uint16_t divider)
{
    ...
    {
        case FDC2214_Channel_0:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_CLOCK_DIVIDERS_CH0, temp);
        break;

        case FDC2214_Channel_1:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_CLOCK_DIVIDERS_CH1, temp);
        break;

        case FDC2214_Channel_2:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_CLOCK_DIVIDERS_CH2, temp);
        break;

        case FDC2214_Channel_3:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_CLOCK_DIVIDERS_CH3, temp);
        break;
    }
}
...
void FDC2214_SetINTB(uint8_t mode)
{
    ...
    temp = FDC2112_Read_Reg(FDC2214_Addr, FDC2214_CONFIG);
    temp &= 0xFF7F;
    if(mode)
        temp |= 0x0080;
    FDC2112_Set_Reg(FDC2214_Addr, FDC2214_CONFIG, temp);

    temp = FDC2112_Read_Reg(FDC2214_Addr, FDC2214_ERROR_CONFIG);
    temp &= 0xFFFE;
    if(mode)
        temp |= 0x0001;
    FDC2112_Set_Reg(FDC2214_Addr, FDC2214_ERROR_CONFIG, temp);
}
...
void FDC2214_SetMUX_CONFIG(uint8_t autoscan, FDC2214_channel_sequence_t channels, FDC2214_filter_bandwidth_t bandwidth)
{
    ...
    FDC2112_Set_Reg(FDC2214_Addr, FDC2214_MUX_CONFIG, temp);
}
...
{
	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_RESET_DEV, 0x8000);
}
...
void FDC2214_SetDriveCurrent(FDC2214_channel_t channel, FDC2214_drive_current_t current)
{
    ...
    {
        case FDC2214_Channel_0:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_DRIVE_CURRENT_CH0, temp);
        break;

        case FDC2214_Channel_1:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_DRIVE_CURRENT_CH1, temp);
        break;

        case FDC2214_Channel_2:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_DRIVE_CURRENT_CH2, temp);
        break;

        case FDC2214_Channel_3:
        	FDC2112_Set_Reg(FDC2214_Addr, FDC2214_DRIVE_CURRENT_CH3, temp);
        break;
    }
}
 ...
 *  @return     外部电容大小，单位pF
 *  @since      v1.0
 *  Sample usage:       FDC2214_CalculateCapacitance(FDC2214_Channel_0, 6000*000, 18, 33);
 */
double FDC2214_CalculateCapacitance(double frequency, float inductance, float capacitance)
{
    double cap = 0.0;

    cap = (double)(1000000000000000000 / (inductance * (2 * PI * frequency) * (2 * PI * frequency)));
...
{
    uint16_t deviceID = 0;
    deviceID = FDC2112_Read_Reg(FDC2214_Addr, FDC2214_DEVICE_ID);//读器件ID
    if(deviceID == FDC2214_ID)
    {
        ...
        FDC2214_SetChannelClock(FDC2214_Channel_0, 0x02, 0x01);//设置时钟分频，1分频，40MHz


        FDC2214_SetINTB(0);//关闭INTB中断
        FDC2214_SetActiveChannel(FDC2214_Channel_0);//开启通道0
        FDC2214_SetCurrentMode(0);//全电流激活模式
        FDC2214_SetClockSource(0);//内部时钟源
        FDC2214_SetHighCurrentMode(0);//用>1.5mA的电流驱动

        FDC2214_SetMUX_CONFIG(1, FDC2214_Channel_Sequence_0_1_2_3, FDC2214_Bandwidth_10M);//打开自动扫描，10MHz带宽

        FDC2214_SetDriveCurrent(FDC2214_Channel_0, FDC2214_Drive_Current_0_644);//通道0驱动电流0.0164mA
