...
// 全局状态变量，初始化
static SSI_State ssi_state =
{ .retry_count = 0, .fatal_error = false, .htim = &ENCODER_TIM_HANDLE };
// SPI接收缓冲区（存放4字节原始数据）
...
static SSI_Data_t Parse_SSI_Data(const uint8_t *rxData)
{
	SSI_Data_t result;
	// 组合18位原始数据
	uint32_t raw_data = (((rxData[0] & 0x7F) << 11) | (rxData[1] << 3)
			| (rxData[2] >> 5));
 ...
 */
static SSI_Data_t Process_SSI_Frame(uint8_t *rx_data, GPIO_TypeDef *cs_port,
		uint16_t cs_pin)
{
	// 关闭片选信号
