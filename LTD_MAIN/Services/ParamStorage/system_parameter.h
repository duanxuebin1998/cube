...
#define MAX_WEIGHT 32768 //称重传感器最大承重值
...
typedef struct
{
	...
	CommandType current_command; // 待执行指令
	uint32_t sensor_position;	 // 传感器实时位置
...
typedef struct
{
	uint32_t current_encoder_turns; ///< 当前编码器圈数
	uint32_t current_encoder_value; ///< 当前编码值
	uint32_t sensor_position;       ///< 传感器位置
	uint32_t frequency;             ///< 频率
	uint32_t temperature;           ///< 温度 (单位: 0.01°C)
	uint32_t air_frequency;         ///< 空气中频率
	uint32_t current_amplitude;     ///< 当前幅值
//    uint32_t current_x_angle;       ///< 当前X轴的角度
//    uint32_t current_y_angle;       ///< 当前Y轴的角度
...
typedef struct
{
	...
	uint32_t tank_bottom_pressure;				 ///< 罐底压力
	uint32_t gas_phase_pressure;				 ///< 气相压力
...
typedef struct
{
	...
	uint32_t circumference;	   // 导论周长
	uint32_t presetZeroEncoder; // 预设零点编码
	uint32_t presetZeroAngle;   // 预设零点编码角度
...
//
// #define SOFTVERSION 1569//程序版本号
//
// extern struct SystemParameterS systemparameter;
//
// int SystemParameterInit(void);
// int SystemParameterWriteAll(void);
// int SystemParameterWriteSingle(int data,int address);
//
// u32 UpdateAlarm(u32 tmplevel);
// void SystemParameterSersion(void);
// void SysParameterWhenFactory(void);
// int ReadParameterPowerOn(void);
// int FactoryDataReset(void);
// int ConfigurationFileReset(void);
// int UpdateSoftVersion(void);
