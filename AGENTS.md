...
- 本工作区提交门禁放在本地 `tools/git-hooks/pre-commit` 和 `tools/git-hooks/commit-msg`；提交命令必须显式设置环境变量 `CUBE_COMMIT_SCOPE=all` 或 `CUBE_COMMIT_SCOPE=scoped`。使用 `scoped` 时还必须设置 `CUBE_COMMIT_SCOPE_FILE=<任务JSON>`，由门禁按其中 `task.writePaths` 或 `task.allowedPaths` 核对暂存范围。`pre-commit` 检查范围，`commit-msg` 同时检查版本标题和四段式正文。当前工作区应保持 `git config core.hooksPath tools/git-hooks`；新克隆需先从本地同步源恢复 `tools/`，再配置 hooksPath 后提交。
